...
from OpenTron.core.registry import SpeechRegistry
from OpenTron.speech.faster_whisper import FasterWhisperBackend
...
def test_faster_whisper_transcribe():
    ...
    from OpenTron.speech._stubs import TranscriptionResult
    ...
    with patch(
        "OpenTron.speech.faster_whisper.WhisperModel",
...
def test_faster_whisper_falls_back_from_unsupported_float16():
    ...
    with (
        ...
        patch(
            "OpenTron.speech.faster_whisper.ctranslate2",
            MagicMock(
...
def test_faster_whisper_missing_dependency_hint_uses_desktop_extra():
    with patch("OpenTron.speech.faster_whisper.WhisperModel", new=None):
        backend = FasterWhisperBackend()
...
def test_faster_whisper_supported_formats():
    ...
    with patch("OpenTron.speech.faster_whisper.WhisperModel"):
        from OpenTron.speech.faster_whisper import FasterWhisperBackend
