...
from OpenTron.agents.native_react import REACT_SYSTEM_PROMPT, NativeReActAgent
...
class TestSystemBuilderCapturesFewShot:
    def test_skill_manager_examples_stored_on_system(self, tmp_path):
        ...
        stashes them on the TronSystem instance for _run_agent to
        forward to tool-using agents."""

        from OpenTron.skills.manager import SkillManager
...
class TestRunAgentForwardsExamples:
    def test_run_agent_passes_examples_to_tool_using_agent(self):
        ...
        # Build a minimal TronSystem with the captured examples
        system = TronSystem.__new__(TronSystem)
        ...
        with patch(
            "OpenTron.core.registry.AgentRegistry.get",
