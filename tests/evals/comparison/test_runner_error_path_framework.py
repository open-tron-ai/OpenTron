...
default of ``"OpenTron"`` to silently mislabel data from foreign
...
from OpenTron.evals.core.dataset import DatasetProvider
from OpenTron.evals.core.runner import EvalRunner
from OpenTron.evals.core.scorer import Scorer
from OpenTron.evals.core.types import EvalRecord, RunConfig
...
class TestErrorPathFrameworkPropagation:
    def test_failed_task_keeps_backend_framework_name(self, tmp_path: Any) -> None:
        ...
        backend's framework_name, not the dataclass default 'OpenTron'."""
        cfg = RunConfig(
            benchmark="mock",
            backend="hermes",
            model="mock-model",
        ...
        # NOT from the EvalResult dataclass default of "OpenTron".
    ...
    def test_default_framework_when_backend_missing_attr(self, tmp_path: Any) -> None:
        ...
        a sensible default (the ABC's "OpenTron"), via getattr fallback."""
        ...
        # the conservative "OpenTron" tag rather than crashing.
        assert results[0].framework == "OpenTron"
