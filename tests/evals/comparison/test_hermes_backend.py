"""Tests for OpenTron.evals.backends.external.hermes_agent.HermesBackend."""
...
from OpenTron.evals.backends.external._subprocess_runner import SubprocessResult
from OpenTron.evals.backends.external.hermes_agent import HermesBackend
    ...
    return ThirdPartyConfig(
        entries={
            "hermes": ThirdPartyEntry(
                ...
                runner_script=(
                    "src/OpenTron/evals/backends/external/_runners/hermes_runner.py"
                ),
                python_executable="",
            )
        }
...
class TestHermesBackend:
    def test_generate_full_builds_correct_subprocess_command(
        ...
        with (
            patch(
                "OpenTron.evals.backends.external.hermes_agent.load_third_party_config",
                return_value=cfg,
            ),
            patch(
                "OpenTron.evals.backends.external.hermes_agent.verify_commit_pin",
            ),
            patch(
                "OpenTron.evals.backends.external.hermes_agent.run_one_shot",
            ) as mock_run,
        ):
