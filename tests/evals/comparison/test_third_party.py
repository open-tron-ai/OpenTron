"""Tests for OpenTron.evals.comparison.third_party."""
...
class TestVerifyCommitPin:
    def test_matching_commit_passes(self, tmp_path: Path) -> None:
        ...
        from OpenTron.evals.comparison.third_party import verify_commit_pin
    ...
    def test_drift_override_env_var(
        ...
        monkeypatch.setenv("Tron_ALLOW_COMMIT_DRIFT", "1")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value.stdout = "deadbeef\n"
            mock_run.return_value.returncode = 0
