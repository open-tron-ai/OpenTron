"""Verify TronDirectBackend.generate_full returns the spec §6.2 extended fields."""
...
class TestTronDirectExtendedFields:
    def test_generate_full_includes_framework_and_commit(self) -> None:
        ...
        # engine); set required attrs directly. TronDirectBackend.generate_full
        # calls ``self._system.engine.generate(messages, ...)`` so we mock the
        # whole ``_system`` chain.
        backend = TronDirectBackend.__new__(TronDirectBackend)
        ...
        assert result["framework"] == "OpenTron"
