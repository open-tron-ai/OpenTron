...
from OpenTron.server.api_routes import include_all_routes  # noqa: E402
...
class TestMemoryRustMissing:
    """Regression for #502: when the native ``OpenTron_rust`` extension is
    ...
    def _client(monkeypatch):
        ...
        def _boom():
            raise ImportError("No module named 'OpenTron_rust'")

        import OpenTron._rust_bridge as bridge
    ...
    def test_store_is_not_a_silent_noop(self, monkeypatch):
        ...
        assert "OpenTron_rust" in detail
    ...
    def test_config_reports_unavailable(self, monkeypatch):
        ...
        assert "OpenTron_rust" in (data["detail"] or "")
...
class TestMetricsRoute:
    def test_metrics_endpoint(self):
        ...
        assert resp.status_code == 200
        assert "OpenTron" in resp.text or "No metrics" in resp.text
