...
def test_connect_slack_bot_token_returns_400(app, tmp_path: Path) -> None:
    ...
    from OpenTron.connectors.slack_connector import SlackConnector
    from OpenTron.server.connectors_router import _instances
...
def test_connect_granola_invalid_key_returns_400_keeps_existing(
    ...
    from OpenTron.connectors.granola import GranolaConnector, GranolaKeyError
    from OpenTron.server.connectors_router import _instances
    ...
    try:
        with patch(
            "OpenTron.connectors.granola._granola_api_validate_key",
