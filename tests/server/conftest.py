...
default) wires a ``TraceStore`` at the real ``~/.OpenTron/traces.db``. Now
...
def _isolate_traces_db(tmp_path, monkeypatch):
    ...
    ``load_config`` returns a fresh ``TronConfig`` per call (no caching), so
    wrapping it to rewrite ``traces.db_path`` only affects calls made during
    the test — there is no global leak.
    """
    from OpenTron.core import config as _config
