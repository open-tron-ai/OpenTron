...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("OpenTron.mcp.transport.StreamableHTTPTransport"),
        patch("OpenTron.mcp.client.MCPClient"),
        patch("OpenTron.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
