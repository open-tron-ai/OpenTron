"""Tests for OpenTron.cli._first_run.check_and_route."""
...
from OpenTron.cli import _first_run
...
def test_passes_through_when_subcommand_present(tmp_OpenTron_home: Path) -> None:
    """If user typed `Tron ask ...`, guard is a no-op."""
...
def test_routes_to_chat_when_config_exists(tmp_OpenTron_home: Path) -> None:
    (tmp_OpenTron_home / "config.toml").write_text('[engine]\ndefault = "ollama"\n')
...
def test_routes_to_init_when_no_config(tmp_OpenTron_home: Path) -> None:
    ctx = _ctx_with_invocation(None)
    _first_run.check_and_route(ctx)
    assert ctx.invoke.called
    invoked_cmd = ctx.invoke.call_args[0][0]
    assert invoked_cmd.name == "init"
    # Cold-path init must run with the from-bare-Tron flag set.
    assert ctx.invoke.call_args.kwargs.get("from_bare_Tron") is True


def test_handles_missing_state_dir(tmp_path: Path, monkeypatch) -> None:
    """When ~/.OpenTron doesn't exist at all, route to init."""
    fresh_home = tmp_path / "fresh"
    monkeypatch.setattr("OpenTron.core.config.DEFAULT_CONFIG_DIR", fresh_home)
    monkeypatch.setattr(
        "OpenTron.core.config.DEFAULT_CONFIG_PATH", fresh_home / "config.toml"
    )
    ctx = _ctx_with_invocation(None)
    _first_run.check_and_route(ctx)
    invoked_cmd = ctx.invoke.call_args[0][0]
...
def test_root_group_invokes_guard_on_bare_Tron(
    tmp_OpenTron_home: Path, monkeypatch
) -> None:
    """End-to-end: bare `Tron` invocation calls the first-run guard.
    ...
    monkeypatch.setattr("OpenTron.cli._first_run.check_and_route", _recorder)
