...
class TestGEPAOptimizerConfig:
    def test_default_config(self) -> None:
        from OpenTron.core.config import GEPAOptimizerConfig
    ...
    def test_optimizer_init(self) -> None:
        from OpenTron.core.config import GEPAOptimizerConfig
        from OpenTron.learning.agents.gepa_optimizer import GEPAAgentOptimizer
...
class TestGEPAOptimizerOptimize:
    ...
    def test_no_gepa_reports_error(self) -> None:
        ...
        with patch.dict("sys.modules", {"gepa": None}):
            with patch(
                "OpenTron.learning.agents.gepa_optimizer.HAS_GEPA",
                False,
            ):
                result = optimizer.optimize(mock_store)
...
class TestOpenTronGEPAAdapter:
    def test_adapter_init(self) -> None:
        from OpenTron.core.config import GEPAOptimizerConfig
        from OpenTron.learning.agents.gepa_optimizer import (
            OpenTronGEPAAdapter,
        )

        mock_store = MagicMock()
        adapter = OpenTronGEPAAdapter(
            mock_store,
            "native_react",
            GEPAOptimizerConfig(),
        )
