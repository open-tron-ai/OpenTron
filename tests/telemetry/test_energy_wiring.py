...
from OpenTron.core.types import Message, Role, TelemetryRecord
from OpenTron.telemetry.aggregator import AggregatedStats, TelemetryAggregator
...
_bench_mod = importlib.import_module("OpenTron.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a TronConfig with energy monitoring enabled."""
    cfg = TronConfig()
    ...
    # `Tron ask` (no --agent) now falls back to ``agent.default_agent``
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "OpenTron.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Tron instance."""
        from OpenTron.sdk import Tron
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "OpenTron.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "OpenTron.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "OpenTron.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "OpenTron.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
