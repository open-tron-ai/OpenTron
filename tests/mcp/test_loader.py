"""Regression tests for OpenTron.mcp.loader.load_mcp_tools_from_config.

Closes the gap that #461 surfaced — MCP tools were silently dropped on
`Tron ask` and `Tron serve` because neither path read
...
def _mock_mcp_stack():
    ...
    with patch("OpenTron.mcp.client.MCPClient") as MockClient, patch(
        "OpenTron.mcp.transport.StreamableHTTPTransport"
    ) as MockHttp, patch(
        "OpenTron.mcp.transport.StdioTransport"
    ) as MockStdio, patch(
        "OpenTron.tools.mcp_adapter.MCPToolProvider"
    ) as MockProvider:
...
class TestLoaderEarlyReturns:
    def test_disabled_returns_empty(self, _mock_mcp_stack):
        from OpenTron.mcp.loader import load_mcp_tools_from_config
