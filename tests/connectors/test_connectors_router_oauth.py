...
``~/.OpenTron/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    test leaves ``~/.OpenTron`` untouched.
    ...
    *source* (``OpenTron.core.config``) — every connector re-derives
    ...
    import OpenTron.connectors.oauth as oauth_mod
    import OpenTron.core.config as config_mod
    import OpenTron.server.connectors_router as router_mod
    ...
    google_mods = [
        "OpenTron.connectors.gdrive",
        "OpenTron.connectors.gcalendar",
        "OpenTron.connectors.gcontacts",
        "OpenTron.connectors.gmail",
        "OpenTron.connectors.google_tasks",
    ]
    ...
    # subsequent tests in the same process see ~/.OpenTron again.
    for name in google_mods:
        if name in sys.modules:
            importlib.reload(sys.modules[name])
...
def client(hermetic_connectors: Path) -> Iterator[TestClient]:
    from OpenTron.server.connectors_router import create_connectors_router
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("OpenTron.core.open_browser") as mock_browser:
        resp = client.post(
...
def test_oauth_callback_exchanges_and_connects(
    ...
    from OpenTron.connectors.gdrive import GDriveConnector
