...
from OpenTron.cli._install_detect import InstallInfo, detect_install


def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``OpenTron.__file__`` at ``tmp_path / OpenTron / __init__.py``."""
    pkg_dir = tmp_path / "OpenTron"
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "OpenTron" / "lib" / "python3.12"
    fake.mkdir(parents=True)
    _patch_pkg_file(fake, monkeypatch)

    info = detect_install()
    assert info.kind == "uv-tool"
    assert info.upgrade_command == "uv tool upgrade OpenTron"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade OpenTron"
...
def test_missing_OpenTron_file_falls_back_to_pypi(monkeypatch):
    """OpenTron unimportable / no __file__ — still get a sane default."""
    with patch("OpenTron.cli._install_detect.Path") as mock_path:
