"""Tests for ``Tron doctor`` CLI command."""
...
from OpenTron.cli.doctor_cmd import (
    CheckResult,
...
class TestDoctorRuns:
    def test_doctor_runs(self) -> None:
        ...
        with (
            patch("OpenTron.cli.doctor_cmd.load_config", return_value=mock_config),
            patch(
                "OpenTron.cli.doctor_cmd.DEFAULT_CONFIG_PATH",
                Path("/tmp/nonexistent/config.toml"),
            ),
            patch("OpenTron.cli.doctor_cmd._check_engines", return_value=[]),
            patch("OpenTron.cli.doctor_cmd._check_models", return_value=[]),
            patch(
                "OpenTron.cli.doctor_cmd._check_speech_backend",
...
class TestCheckEngineProbing:
    def test_check_engine_probing(self) -> None:
        ...
        from OpenTron.cli.doctor_cmd import CheckResult
...
class TestCheckDefaultModel:
    def test_empty_default_model_is_not_a_warning(self) -> None:
        ...
        mock_config = MagicMock()
        mock_config.intelligence.default_model = ""
        with patch("OpenTron.cli.doctor_cmd.load_config", return_value=mock_config):
...
class TestCheckSpeechBackend:
    def test_check_speech_backend_ready(self) -> None:
        ...
        with patch(
            "OpenTron.speech._discovery.get_speech_backend",
