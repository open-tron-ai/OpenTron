"""Tests for ``Tron ask --agent`` CLI integration."""
...
from OpenTron.agents._stubs import AgentContext, AgentResult, ToolUsingAgent
from OpenTron.cli import cli
from OpenTron.core.types import ToolCall, ToolResult
from OpenTron.tools._stubs import BaseTool, ToolSpec

_ask_mod = importlib.import_module("OpenTron.cli.ask")
...
def agent_setup():
    ...
    from OpenTron.core.registry import AgentRegistry, ToolRegistry
...
def mock_setup():
    ...
    with (
        ...
        mock_cfg.return_value = TronConfig()
...
class TestAskAgentOption:
    ...
    def test_no_agent_flag_falls_back_to_config_default_agent(
        ...
        The default ``TronConfig`` sets ``default_agent = "simple"``, so
        ``Tron ask "..."`` should route through SimpleAgent rather than
...
class TestBuildTools:
    def test_build_calculator(self, mock_setup):
        from OpenTron.cli.ask import _build_tools
        from OpenTron.core.config import TronConfig
...
class TestPersonaFilesReachModel:
    ...
    def test_soul_md_content_reaches_engine_in_simple_agent(
        ...
        soul.write_text("PERSONA_SENTINEL_zh_Tron", encoding="utf-8")
        ...
        assert "PERSONA_SENTINEL_zh_Tron" in joined
