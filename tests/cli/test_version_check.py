...
from OpenTron.cli import _version_check
from OpenTron.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("OpenTron_NO_UPDATE_CHECK", "CI", "OpenTron_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.OpenTron state.
    monkeypatch.setenv("OpenTron_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_Tron_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("OpenTron_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("OpenTron_CONFIG", str(cfg))
    ...
    def test_OpenTron_config_env_override(self, monkeypatch, tmp_path):
        """OpenTron_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("OpenTron.cli._version_check._do_check")
    ...
    @patch(
        "OpenTron.cli._version_check._do_check",
