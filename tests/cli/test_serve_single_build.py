"""Regression tests for #263 — ``Tron serve`` must build the system once.
...
1. ``SystemBuilder.build`` is never called during ``Tron serve`` startup
...
# ``OpenTron.cli.serve`` as an attribute resolves to the click *command*
# (re-exported on the package); grab the real module to monkeypatch its globals.
serve_mod = importlib.import_module("OpenTron.cli.serve")
...
def _repopulate_registries() -> None:
    ...
    import OpenTron.agents  # noqa: F401
    import OpenTron.tools  # noqa: F401
    import OpenTron.tools.storage  # noqa: F401
    from OpenTron.core.registry import (
        AgentRegistry,
        MemoryRegistry,
        ToolRegistry,
    )
    ...
    if not AgentRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("OpenTron.agents.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("OpenTron.tools.")
...
def _run_serve(tmp_path, monkeypatch, *, build_spy, set_system_spy):
    """Invoke ``Tron serve`` with all heavy/blocking pieces stubbed out.
    ...
    monkeypatch.setattr("OpenTron.security.setup_security", lambda *a, **k: sec)

    with (
        patch(
            "OpenTron.system.builder.SystemBuilder.build",
            build_spy,
        ),
        patch(
            "OpenTron.agents.executor.AgentExecutor.set_system",
            set_system_spy,
        ),
...
def test_serve_does_not_call_systembuilder_build(tmp_path, monkeypatch):
    ...
    build_spy = MagicMock(
        side_effect=AssertionError(
            "SystemBuilder.build() must not run during `Tron serve` startup "
