"""Tests for ``Tron chat`` interactive REPL command."""
...
from OpenTron.agents._stubs import (
    AgentContext,
    AgentResult,
    BaseAgent,
    ToolUsingAgent,
)
from OpenTron.cli.chat_cmd import _read_input, chat
...
class TestChatAgents:
    def test_simple_agent_does_not_receive_tool_only_kwargs(self) -> None:
        ...
        with (
            patch("OpenTron.cli.chat_cmd.load_config", return_value=config),
            patch("OpenTron.engine.get_engine", return_value=("mock", engine)),
            patch("OpenTron.intelligence.register_builtin_models"),
        ):
