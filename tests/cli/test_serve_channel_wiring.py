"""Tests for TronSystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on TronSystem directly. The serve.py
...
from OpenTron.sessions.session import SessionStore
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> TronSystem:
    """Build a minimal TronSystem with mock engine for testing wire_channel."""
    config = TronConfig()
...
# ---------------------------------------------------------------------------
# Tests via TronSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """TronSystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """TronSystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """TronSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = TronConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
