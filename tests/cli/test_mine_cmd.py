...
def test_pearl_base_model_lookup_uses_public_pearl_ai_artifacts():
    from OpenTron.mining._models import pearl_variant_for_base_model
...
def test_mine_inspect_model_validated_artifact_passes(monkeypatch) -> None:
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd._hf_json", fake_hf_json)
...
def test_mine_init_writes_mining_config(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(
        ...
        env={"OpenTron_CONFIG": str(config_path)},
    )
...
def test_mine_init_writes_cuda_visible_devices_for_vllm(
    ...
    with (
        patch("OpenTron.cli.mine_cmd._detect_hardware"),
        patch(
            "OpenTron.cli.mine_cmd.detect_for_engine_model",
            return_value=MiningCapabilities(supported=True),
        ),
        patch(
            "OpenTron.cli.mine_cmd.check_docker_available",
            return_value=(True, ""),
        ),
        patch("OpenTron.cli.mine_cmd.check_disk_free", return_value=(True, "")),
        patch("OpenTron.cli.mine_cmd._docker_from_env", return_value=MagicMock()),
        patch(
            "OpenTron.cli.mine_cmd.PearlDockerLauncher.ensure_image",
            return_value="OpenTron/pearl-miner:master",
        ),
    ):
        result = CliRunner().invoke(
            cli,
...
"""
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    ...
    with patch("OpenTron.cli.mine_cmd._provider_ids", return_value=("cpu-pearl",)):
        with patch("OpenTron.cli.mine_cmd.MinerRegistry.contains", return_value=True):
            with patch(
                "OpenTron.cli.mine_cmd.MinerRegistry.get",
                ...
                result = CliRunner().invoke(
                    ...
                    ["mine", "start"],
                    env={
                        "OpenTron_CONFIG": str(config_path),
...
def test_mine_status_reports_sidecar_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    monkeypatch.setattr("OpenTron.cli.mine_cmd._pid_alive", lambda pid: True)
    ...
    monkeypatch.setattr(
        "OpenTron.cli.mine_cmd._stats_from_metrics_url",
...
def test_mine_stop_terminates_pids_and_removes_sidecar(
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd._terminate_pid", fake_terminate)
...
def test_mine_doctor_without_config(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", tmp_path / "none.json")

    with patch("OpenTron.cli.mine_cmd._provider_ids", return_value=()):
        result = CliRunner().invoke(
            cli,
            ["mine", "doctor"],
            env={"OpenTron_CONFIG": str(config_path)},
    ...
    assert "Tron mine init" in result.output
...
def test_mine_validate_model_blocks_planned_without_allow(
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    monkeypatch.setattr(
        "OpenTron.cli.mine_cmd._get_json",
...
def test_mine_validate_model_allows_planned_with_runtime_evidence(
    ...
    monkeypatch.setattr(
        "OpenTron.cli.mine_cmd._post_json",
...
def test_mine_validate_model_falls_back_to_vllm_metrics(
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd._stats_from_metrics_url", fake_stats)
