"""Smoke tests for `Tron self-update`.
...
from OpenTron.cli._install_detect import InstallInfo
from OpenTron.cli.self_update_cmd import self_update
...
def _mock_info(kind: str = "pypi") -> InstallInfo:
    return InstallInfo(
        kind=kind,
        upgrade_command={
            "pypi": "pip install --upgrade OpenTron",
            "uv-tool": "uv tool upgrade OpenTron",
            "editable-git": "cd /tmp/repo && git pull && uv sync",
            "unknown": "pip install --upgrade OpenTron",
        }[kind],
    )
...
def test_check_flag_prints_command_and_exits_clean():
    with patch(
        "OpenTron.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ):
        runner = CliRunner()
        result = runner.invoke(self_update, ["--check"])
    assert result.exit_code == 0
    assert "pip install --upgrade OpenTron" in result.output
...
def test_check_does_not_invoke_subprocess():
    with patch(
        "OpenTron.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ), patch("OpenTron.cli.self_update_cmd.subprocess.run") as mock_run:
...
def test_yes_skips_confirmation_and_runs():
    ...
    with patch(
        ...
        "OpenTron.cli.self_update_cmd.subprocess.run",
    ...
    assert args[0] == ["pip", "install", "--upgrade", "OpenTron"]
