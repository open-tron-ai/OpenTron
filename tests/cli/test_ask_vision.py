"""CLI-level regression tests for ``Tron ask`` vision input.
...
from OpenTron.core.types import Role
...
def _patch_ask(monkeypatch, tmp_path: Path, *, engine_name: str) -> _RecordingEngine:
    """Wire ``Tron ask`` to a recording engine reported under ``engine_name``."""
    cfg = TronConfig()
