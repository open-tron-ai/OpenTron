...
from OpenTron.core.config import ChannelConfig, TronConfig, load_config
...
class TestChannelConfigInTronConfig:
    def test_channel_config_in_Tron_config(self) -> None:
        cfg = TronConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_Tron_config_channel_defaults(self) -> None:
        cfg = TronConfig()
