...
def test_config_prefix_prepended(memory_dir: Path):
    ...
    builder = SystemPromptBuilder(
        ...
        system_prompt_config=SystemPromptConfig(prefix="ALWAYS ANSWER AS Tron."),
    )
    prompt = builder.build()
    assert prompt.startswith("ALWAYS ANSWER AS Tron.")
    assert "You are Tron." in prompt
