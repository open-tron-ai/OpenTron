"""Tests for intent-based agent routing in TronSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal TronSystem instance for testing _detect_agent_intent."""
        from OpenTron.system import TronSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = TronSystem.__new__(TronSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("OpenTron.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_Tron_triggers_digest(self, system):
        with patch("OpenTron.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Tron")
