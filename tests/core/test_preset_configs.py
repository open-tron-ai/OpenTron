...
Presets are installed via `Tron init --preset <name>`, which copies
`configs/OpenTron/examples/<name>.toml` to `~/.OpenTron/config.toml`.
...
from OpenTron.core.config import TronConfig, load_config
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `Tron init` expects to be populated for a working first run.
