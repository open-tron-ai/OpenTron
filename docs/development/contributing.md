...
```bash
uv run Tron --version   # Should print 0.1.0
uv run Tron --help      # Show all subcommands
```
...
OpenTron uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=OpenTron --cov-report=html
```
...
OpenTron uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/OpenTron/`:

```
src/OpenTron/
    __init__.py                 # Package root, __version__
    sdk.py                      # Tron class — high-level Python SDK

    core/                       # Shared infrastructure
        config.py               # TronConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # Tron ask
        init_cmd.py             # Tron init
        model.py                # Tron model list/info
        memory_cmd.py           # Tron memory index/search/stats
        telemetry_cmd.py        # Tron telemetry stats/export/clear
        bench_cmd.py            # Tron bench run
        serve.py                # Tron serve
```
...
try:
    import OpenTron.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
