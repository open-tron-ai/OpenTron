...
prove OpenTron wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through OpenTron.
...
- Docker container `OpenTron-pearl-miner` starts.
- `~/.OpenTron/runtime/mining.json` is written.
...
```bash
docker ps --filter name=OpenTron-pearl-miner
cat ~/.OpenTron/runtime/mining.json
uv run Tron mine logs --tail 200
...
## Verify OpenTron Inference Uses Mining Endpoint

Run:

```bash
uv run Tron mine doctor
uv run Tron ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/OpenTron/mining/_metrics.py`.
...
```bash
uv run Tron mine stop
docker ps --filter name=OpenTron-pearl-miner
test ! -e ~/.OpenTron/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `Tron ask` no longer routes through `vllm-pearl-mining` unless another
...
- OpenTron inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run Tron mine doctor
uv run Tron mine status || true
uv run Tron mine logs --tail 300 || true
docker inspect OpenTron-pearl-miner || true
