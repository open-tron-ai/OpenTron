...
2. **`Tron` (no args)** drops into a chat session within 2 s.
...
6. **`Tron doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`Tron-uninstall`** removes `~/.OpenTron/` and `~/.local/bin/Tron*`. Verify with `ls`.
...
- Delete `~/.OpenTron/config.toml` — verify bare `Tron` re-runs init.
- Delete `~/.OpenTron/.venv` — verify re-running curl heals it.
