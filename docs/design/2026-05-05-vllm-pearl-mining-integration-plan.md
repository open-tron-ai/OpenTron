...
**Goal:** Implement Spec A — the v1 vLLM-Pearl mining integration — as a new `OpenTron.mining` subsystem that lets H100/H200 owners running vLLM solo-mine the Pearl PoUW chain with their normal LLM inference.

**Architecture:** New top-level subsystem `mining/` mirrors OJ's existing primitive pattern (peer to `engine/`, `agents/`). A `MiningProvider` ABC + `MinerRegistry` enables future hardware/engine paths (Apple Silicon, AMD, Ollama) without rewrite. The v1 `vllm-pearl` provider orchestrates Pearl's published Docker container; a runtime sidecar at `~/.OpenTron/runtime/mining.json` decouples mining lifecycle from the existing vLLM engine class. Three deliberate seams (`submit_target` tagged-union, zero-valued `fee_bps`/`fees_owed`, reserved `mining/pools/`) leave room for v2 pool support without retrofit pain.
...
- Absolute imports only (`from OpenTron.core.registry import ...`).
...
| Path | Responsibility |
|---|---|
| `src/OpenTron/mining/__init__.py` | Package init; soft-imports providers via `try/except ImportError` |
| `src/OpenTron/mining/_stubs.py` | `MiningProvider` ABC, `MiningCapabilities`, `MiningConfig`, `MiningStats`, `SoloTarget`/`PoolTarget` tagged union, `Sidecar` dataclass + read/write helpers |
| `src/OpenTron/mining/_discovery.py` | `detect_providers()`, hardware/Docker/disk/pearld checks, wallet-format check |
| `src/OpenTron/mining/_constants.py` | `PEARL_REPO`, `PEARL_PINNED_REF`, `PEARL_IMAGE_TAG`, default ports, default model, sidecar path constants |
| `src/OpenTron/mining/_docker.py` | `PearlDockerLauncher` — `ensure_image()`, `start()`, `stop()`, `is_running()`, `get_logs()` |
| `src/OpenTron/mining/_collector.py` | `MiningTelemetryCollector` — full impl, shipped unwired in v1; v1.x lights it up in the gateway daemon |
| `src/OpenTron/mining/vllm_pearl.py` | `VllmPearlProvider` (`MiningProvider` impl); `_parse_gateway_metrics()`; `ensure_registered()` |
| `src/OpenTron/mining/pools/__init__.py` | Reserved location for v2 `PoolClient` work — empty except for a docstring saying so |
| `src/OpenTron/cli/mine_cmd.py` | `Tron mine` Click group: `init`, `start`, `stop`, `status`, `doctor`, `attach`, `logs` |
...
| Path | Change |
|---|---|
| `src/OpenTron/core/registry.py` | Add `MinerRegistry` class + entry in `__all__` |
| `src/OpenTron/core/config.py` | Add `MiningConfig` field to `TronConfig`; add TOML parser for `[mining]` with `submit_target` tagged-union resolution |
| `src/OpenTron/engine/_discovery.py` | Add sidecar-aware engine resolution: when `~/.OpenTron/runtime/mining.json` exists, register a derived `vllm` engine pointing at `vllm_endpoint` |
| `src/OpenTron/telemetry/store.py` | Add nullable `mining_session_id` column to inference rows; bump `PRAGMA user_version`; helper to tag rows when sidecar is present |
| `src/OpenTron/cli/__init__.py` | Register the `mine` Click group |
| `src/OpenTron/cli/hints.py` | Add hint: "mining configured but not running — start it with `Tron mine start`" |
...
**Files:**
- Modify: `src/OpenTron/core/registry.py`
...
def test_miner_registry_register_and_get():
    from OpenTron.core.registry import MinerRegistry
...
class MinerRegistry(RegistryBase[Any]):
    ...
    ``OpenTron.mining._stubs``. Registry keys are short lowercase strings
...
```bash
git add src/OpenTron/core/registry.py tests/conftest.py tests/core/test_registry.py
...
**Files:**
- Create: `src/OpenTron/mining/__init__.py`
- Create: `src/OpenTron/mining/_constants.py`
- Create: `src/OpenTron/mining/_stubs.py`
- Create: `src/OpenTron/mining/pools/__init__.py`
...
from OpenTron.core.config import GpuInfo, HardwareInfo
...
def test_mining_capabilities_default_unsupported():
    from OpenTron.mining._stubs import MiningCapabilities
...
def test_solo_target_dataclass():
    from OpenTron.mining._stubs import SoloTarget
...
def test_pool_target_dataclass():
    from OpenTron.mining._stubs import PoolTarget
...
def test_mining_config_v1_defaults():
    from OpenTron.mining._stubs import MiningConfig, SoloTarget
    cfg = MiningConfig(
        provider="vllm-pearl",
...
def test_mining_stats_v1_defaults():
    from OpenTron.mining._stubs import MiningStats
...
def test_mining_provider_is_abstract():
    from OpenTron.mining._stubs import MiningProvider
...
def test_sidecar_write_then_read_roundtrip(sidecar_path: Path, sample_sidecar_payload: dict):
    from OpenTron.mining._stubs import Sidecar
...
```
Expected: ALL fail with `ImportError` on `OpenTron.mining._stubs`.

- [ ] **Step 5: Create `_constants.py`**

```python
# src/OpenTron/mining/_constants.py
...
PEARL_IMAGE_TAG = f"OpenTron/pearl-miner:{PEARL_PINNED_REF}"
...
RUNTIME_DIR = Path.home() / ".OpenTron" / "runtime"
...
PEARL_CACHE_DIR = Path.home() / ".OpenTron" / "cache" / "pearl"
```

- [ ] **Step 6: Create `_stubs.py`**

```python
# src/OpenTron/mining/_stubs.py
...
from OpenTron.core.config import HardwareInfo
...
class MiningCapabilities:
    ...
    ``reason`` is human-readable and surfaced verbatim by ``Tron mine doctor``
...
class MiningProvider(ABC):
    ...
    def detect(cls, hw: HardwareInfo, engine_id: str, model: str) -> MiningCapabilities:
        ...
        by ``Tron mine doctor`` and ``Tron mine init`` for fast capability
        reporting.
        """
...
class Sidecar:
    """Read/write helpers for ``~/.OpenTron/runtime/mining.json``."""
...
```python
# src/OpenTron/mining/__init__.py
...
from OpenTron.mining._stubs import (
    MiningCapabilities,
    MiningConfig,
    MiningProvider,
    MiningStats,
...
try:
    from OpenTron.mining import vllm_pearl  # noqa: F401
...
```python
# src/OpenTron/mining/pools/__init__.py
...
```bash
git add src/OpenTron/mining/ tests/mining/__init__.py tests/mining/conftest.py tests/mining/test_stubs.py
git commit -m "feat(mining): add ABC, dataclasses, constants, sidecar IO"
```

---

## Task 3 — `MiningConfig` integration in `TronConfig` + TOML parsing

**Files:**
- Modify: `src/OpenTron/core/config.py`
...
provider       = "vllm-pearl"
wallet_address = "prl1qexampleaddress"
submit_target  = "pool:https://pool.OpenTron.ai/submit"
...
def test_mining_config_absent_means_none(tmp_path):
    from OpenTron.core.config import load_config
...
def test_mining_config_pool_parsed_as_pool_target(tmp_path):
    ...
    assert cfg.mining.submit_target.url == "https://pool.OpenTron.ai/submit"
```
...
- [ ] **Step 4: Add `mining` field to `TronConfig`**

In `src/OpenTron/core/config.py`, add an import near the top:

```python
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from OpenTron.mining._stubs import MiningConfig
```

Add the field to `TronConfig` (insert in alphabetical order — after `memory_files`, before `operators`):
...
In `src/OpenTron/core/config.py`, locate the existing `load_config()` function (around line 1541). Find the section that processes the loaded TOML data into the `TronConfig` (look for where it iterates the `data` dict and assigns to dataclass fields). Add a dedicated handler for `mining` because of the tagged-union parsing — it can't go through the generic walker.
...
def _parse_mining_section(data: dict) -> Optional["MiningConfig"]:
    ...
    from OpenTron.mining._stubs import MiningConfig, PoolTarget, SoloTarget
...
```bash
git add src/OpenTron/core/config.py tests/core/test_config.py tests/mining/fixtures/
git commit -m "feat(mining): integrate MiningConfig into TronConfig with TOML parsing"
```
...
**Files:**
- Create: `src/OpenTron/mining/_discovery.py`
...
def test_detect_supported_on_h100(hopper_hw):
    from OpenTron.mining._discovery import detect_for_engine_model
...
def test_check_docker_available_true():
    from OpenTron.mining._discovery import check_docker_available
    with patch("OpenTron.mining._discovery._docker_client") as fake:
...
def test_check_disk_free_passes(tmp_path):
    from OpenTron.mining._discovery import check_disk_free
    with patch("OpenTron.mining._discovery.shutil.disk_usage") as du:
        # 500 GB free
...
def test_check_pearld_reachable_true():
    from OpenTron.mining._discovery import check_pearld_reachable
    with patch("OpenTron.mining._discovery.httpx.post") as post:
...
def test_check_wallet_address_format_valid():
    from OpenTron.mining._discovery import check_wallet_address_format
...
```python
# src/OpenTron/mining/_discovery.py
...
def detect_for_engine_model(
    *,
    ...
    ``Tron mine doctor`` and ``Tron mine init``.
    """
...
# ---------------------------------------------------------------------------
# Doctor checks (one per row of `Tron mine doctor` output)
# ---------------------------------------------------------------------------
...
def check_disk_free(path: Path) -> Tuple[bool, str]:
    from OpenTron.mining._constants import MIN_FREE_DISK_GB
...
```bash
git add src/OpenTron/mining/_discovery.py tests/mining/test_discovery.py
...
**Files:**
- Create: `src/OpenTron/mining/_docker.py`
...
def test_ensure_image_already_local():
    from OpenTron.mining._docker import PearlDockerLauncher
    fake = MagicMock()
    fake.images.get.return_value = MagicMock(id="sha256:abc", tags=["OpenTron/pearl-miner:main"])
    launcher = PearlDockerLauncher(client=fake)
    out = launcher.ensure_image("OpenTron/pearl-miner:main")
    assert out == "OpenTron/pearl-miner:main"
    fake.images.get.assert_called_once_with("OpenTron/pearl-miner:main")
...
def test_ensure_image_falls_back_to_build_for_default_tag():
    from OpenTron.mining._docker import PearlDockerLauncher
    from OpenTron.mining._constants import PEARL_IMAGE_TAG
    import docker.errors as derr
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
def test_ensure_image_errors_when_non_default_tag_missing():
    from OpenTron.mining._docker import PearlDockerLauncher, ImageAcquisitionError
    import docker.errors as derr
    import pytest
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
```python
# src/OpenTron/mining/_docker.py
...
from OpenTron.mining._constants import (
    PEARL_CACHE_DIR,
    PEARL_IMAGE_TAG,
    PEARL_PINNED_REF,
    PEARL_REPO,
...
```bash
git add src/OpenTron/mining/_docker.py tests/mining/test_docker.py
...
**Files:**
- Modify: `src/OpenTron/mining/_docker.py`
...
def test_launcher_start_calls_run_with_expected_kwargs(_env_password):
    ...
    cfg = MiningConfig(
        ...
        submit_target=SoloTarget(pearld_rpc_url="http://localhost:44107"),
        extra={
            "docker_image_tag": "OpenTron/pearl-miner:main",
            "model": "pearl-ai/Llama-3.3-70B-Instruct-pearl",
            "vllm_port": 8000,
...
def test_launcher_start_errors_when_password_env_missing():
    from OpenTron.mining._docker import PearlDockerLauncher, ConfigurationError
    from OpenTron.mining._stubs import MiningConfig, SoloTarget
    fake = MagicMock()
    launcher = PearlDockerLauncher(client=fake)
    cfg = MiningConfig(
    ...
    with pytest.raises(ConfigurationError) as ei:
        launcher.start(cfg, image="OpenTron/pearl-miner:main")
...
Append to `src/OpenTron/mining/_docker.py`:
...
class ConfigurationError(RuntimeError):
    ...
    def start(self, config: "MiningConfig", image: str) -> Any:
        ...
        """
        from OpenTron.mining._stubs import MiningConfig  # noqa: F401  (typing only)
        ...
        if password is None:
            raise ConfigurationError(
                ...
                f"set it before running `Tron mine start`"
            )
...
**Files:**
- Modify: `src/OpenTron/mining/vllm_pearl.py` (creating in next task — for now create the parser as a helper)
- Create: `src/OpenTron/mining/_metrics.py`
...
def test_parse_gateway_metrics_full():
    from OpenTron.mining._metrics import parse_gateway_metrics
...
```python
# src/OpenTron/mining/_metrics.py
...
```bash
git add src/OpenTron/mining/_metrics.py tests/mining/test_metrics.py tests/mining/fixtures/
...
**Files:**
- Create: `src/OpenTron/mining/vllm_pearl.py`
...
def test_vllm_pearl_detect_supported_on_h100(hopper_hw):
    from OpenTron.mining.vllm_pearl import VllmPearlProvider
...
async def test_vllm_pearl_start_writes_sidecar(tmp_path, monkeypatch):
    from OpenTron.mining.vllm_pearl import VllmPearlProvider
    from OpenTron.mining._stubs import MiningConfig, SoloTarget, Sidecar

    sidecar_path = tmp_path / "mining.json"
    monkeypatch.setattr(
        "OpenTron.mining.vllm_pearl.SIDECAR_PATH", sidecar_path
    )
...
async def test_vllm_pearl_start_pool_target_raises_not_implemented(monkeypatch, tmp_path):
    from OpenTron.mining.vllm_pearl import VllmPearlProvider
    from OpenTron.mining._stubs import MiningConfig, PoolTarget
    ...
    cfg = MiningConfig(
        provider="vllm-pearl",
        wallet_address="prl1qaaa",
        submit_target=PoolTarget(url="https://pool.OpenTron.ai/submit"),
        extra={"docker_image_tag": "OpenTron/pearl-miner:main"},
    )
...
async def test_vllm_pearl_stop_removes_sidecar(tmp_path, monkeypatch, written_sidecar):
    from OpenTron.mining.vllm_pearl import VllmPearlProvider
    monkeypatch.setattr(
        "OpenTron.mining.vllm_pearl.SIDECAR_PATH", written_sidecar
    )
    fake_client = MagicMock()
    provider = VllmPearlProvider(docker_client=fake_client)
...
def test_vllm_pearl_stats_reads_gateway(monkeypatch, written_sidecar):
    ...
    with patch("OpenTron.mining.vllm_pearl.httpx.get") as get:
...
def test_ensure_registered_is_idempotent():
    from OpenTron.core.registry import MinerRegistry
    from OpenTron.mining.vllm_pearl import (
        VllmPearlProvider,
        ensure_registered,
    )
    ensure_registered()
...
```python
# src/OpenTron/mining/vllm_pearl.py
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see OpenTron#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
...
```bash
git add src/OpenTron/mining/vllm_pearl.py tests/mining/test_vllm_pearl.py
...
**Files:**
- Modify: `src/OpenTron/engine/_discovery.py`
...
Run:
```bash
sed -n '1,80p' src/OpenTron/engine/_discovery.py
```
...
def test_engine_discovery_picks_up_mining_sidecar(tmp_path, monkeypatch, written_sidecar):
    ...
    """
    from OpenTron.engine._discovery import discover_engines
    from OpenTron.mining import _constants as mining_const
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    """
    try:
        from OpenTron.mining import Sidecar
        from OpenTron.mining._constants import SIDECAR_PATH
    except ImportError:
        return
    payload = Sidecar.read(SIDECAR_PATH)
    if payload is None:
    ...
    from OpenTron.core.registry import EngineRegistry
    from OpenTron.engine.openai_compat_engines import OpenAICompatEngine  # adjust to actual class name
...
```bash
git add src/OpenTron/engine/_discovery.py tests/conftest.py tests/mining/conftest.py tests/engine/test_discovery.py
...
**Files:**
- Create: `src/OpenTron/mining/_collector.py`
...
async def test_collector_collect_once_returns_stats(written_sidecar):
    from OpenTron.mining._collector import MiningTelemetryCollector

    sample = (
        "pearl_gateway_shares_submitted_total 50\n"
        "pearl_gateway_shares_accepted_total 49\n"
    )
    with patch("OpenTron.mining._collector.httpx.get") as get:
        get.return_value.status_code = 200
        get.return_value.text = sample
        store = MagicMock()
...
from OpenTron.mining._metrics import parse_gateway_metrics
from OpenTron.mining._stubs import MiningStats, Sidecar
...
```bash
git add src/OpenTron/mining/_collector.py tests/mining/test_collector.py
...
**Files:**
- Modify: `src/OpenTron/telemetry/store.py`
...
Run:
```bash
grep -n "PRAGMA user_version\|CREATE TABLE\|ALTER TABLE\|migrate" src/OpenTron/telemetry/store.py | head -20
```
...
def test_inference_row_has_mining_session_id_column(tmp_path):
    from OpenTron.telemetry.store import TelemetryStore
    db = tmp_path / "tel.db"
...
In `src/OpenTron/telemetry/store.py`:
...
```bash
git add src/OpenTron/telemetry/store.py tests/telemetry/test_store.py
git commit -m "feat(telemetry): add mining_session_id + mining_stats table"
```

---

## Task 12 — `Tron mine doctor` (highest user-facing value, build first)

**Files:**
- Create: `src/OpenTron/cli/mine_cmd.py`
...
def test_mine_doctor_prints_capability_matrix(monkeypatch):
    from OpenTron.cli.mine_cmd import mine
    runner = CliRunner()
    ...
    with patch("OpenTron.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("OpenTron.cli.mine_cmd.check_docker_available", return_value=(True, "running 24.0.7")), \
         patch("OpenTron.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("OpenTron.cli.mine_cmd.check_pearld_reachable", return_value=(True, "block height 442107 (synced)")):
...
def test_mine_doctor_flags_unsupported_hardware():
    ...
    with patch("OpenTron.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("OpenTron.cli.mine_cmd.check_docker_available", return_value=(True, "ok")), \
         patch("OpenTron.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("OpenTron.cli.mine_cmd.check_pearld_reachable", return_value=(False, "connection refused")):
        result = runner.invoke(mine, ["doctor"])
...
```python
# src/OpenTron/cli/mine_cmd.py
"""``Tron mine`` command group.
...
from OpenTron.core.config import HardwareInfo, load_config
from OpenTron.mining._constants import (
    DEFAULT_PEARL_MODEL,
...
from OpenTron.mining._discovery import (
    check_disk_free,
...
def mine() -> None:
    ...
    See https://open-Tron.github.io/OpenTron/user-guide/mining/ for the
    full guide.
    """
...
def doctor() -> None:
    ...
    if mining_cfg is not None:
        ...
        row("pearld", "RPC", False, "no [mining] config — run `Tron mine init`")
...
```bash
git add src/OpenTron/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Tron mine doctor"
```

---

## Task 13 — `Tron mine init / start / stop`

**Files:**
- Modify: `src/OpenTron/cli/mine_cmd.py`
...
def test_mine_start_runs_provider_start(monkeypatch):
    ...
    with patch("OpenTron.cli.mine_cmd.MinerRegistry") as reg, \
         patch("OpenTron.cli.mine_cmd.load_config") as load, \
         patch("OpenTron.cli.mine_cmd.asyncio.run") as arun:
        from OpenTron.mining._stubs import MiningConfig, SoloTarget
...
def test_mine_start_errors_when_no_mining_config():
    from OpenTron.cli.mine_cmd import mine
    runner = CliRunner()
    with patch("OpenTron.cli.mine_cmd.load_config") as load:
...
def init(
    ...
    if not cap.supported:
        raise click.ClickException(
            ...
            f"See `Tron mine doctor` for details."
        )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            f"Set it before `Tron mine start`.",
            err=True,
        )
    ...
    config_path = Path.home() / ".OpenTron" / "config.toml"
...
"""
    ...
    click.echo(f"Done. Run `Tron mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException("no [mining] section in config — run `Tron mine init`")
    ...
    async def _run():
        await provider.start(cfg)
    asyncio.run(_run())
    click.echo(f"Mining started. Run `Tron mine status` for live stats.")
...
```bash
git add src/OpenTron/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Tron mine init/start/stop"
```

---

## Task 14 — `Tron mine status / attach / logs`
...
def test_mine_status_renders_stats(written_sidecar, monkeypatch):
    from OpenTron.cli.mine_cmd import mine
    runner = CliRunner()
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", written_sidecar)
    sample = (
        "pearl_gateway_shares_submitted_total 100\n"
        "pearl_gateway_shares_accepted_total 99\n"
    ...
    with patch("OpenTron.mining.vllm_pearl.httpx.get") as get, \
         patch("OpenTron.cli.mine_cmd.MinerRegistry") as reg:
        get.return_value.status_code = 200
        get.return_value.text = sample
        from OpenTron.mining.vllm_pearl import VllmPearlProvider
...
def test_mine_attach_writes_sidecar(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("OpenTron.cli.mine_cmd.SIDECAR_PATH", sidecar)
...
def test_mine_logs_streams_container_output(monkeypatch):
    ...
    with patch("OpenTron.cli.mine_cmd.PearlDockerLauncher",
...
def status() -> None:
    ...
    cfg = load_config().mining
    if cfg is None:
        raise click.ClickException("no [mining] section — run `Tron mine init`")
    provider_cls = MinerRegistry.get(cfg.provider)
    provider = provider_cls()
    s = provider.stats()
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        container = docker.from_env().containers.get("OpenTron-pearl-miner")
...
```bash
git add src/OpenTron/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Tron mine status/attach/logs"
```
...
**Files:**
- Modify: `src/OpenTron/cli/__init__.py`
- Modify: `src/OpenTron/cli/hints.py`
...
```bash
sed -n '1,50p' src/OpenTron/cli/__init__.py
grep -n "add_command\|@main.command\|main.add_command" src/OpenTron/cli/__init__.py | head -20
```
...
def mining_not_running_hint(cfg, sidecar_present: bool) -> Optional[str]:
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running — start it with `Tron mine start`"
```
...
```bash
git add src/OpenTron/cli/__init__.py src/OpenTron/cli/hints.py tests/cli/test_main.py tests/cli/test_hints.py
...
OpenTron can mine the [Pearl](https://github.com/pearl-research-labs/pearl)
...
uv run Tron mine init    # writes [mining] config + builds Docker image (30-60 min first time)
...
`Tron mine doctor` prints one row per check with a clear ✓ or ✗ and reason.
...
The `OpenTron.mining` subsystem uses a registry pattern identical to
...
1. Create `src/OpenTron/mining/<provider>.py`.
2. Subclass `OpenTron.mining.MiningProvider`.
...
5. Add a soft-import in `mining/__init__.py`:
   ```python
   try:
       from OpenTron.mining import <provider>  # noqa: F401
...
```markdown
- New mining providers must register via `MinerRegistry` in `src/OpenTron/core/registry.py` and expose an idempotent `ensure_registered()` per the autouse-clear test convention.
```
...
```bash
uv run pytest tests/mining/ --cov=OpenTron.mining --cov-report=term-missing
```
Expected: ≥ 80% coverage on the new `mining/` package.

- [ ] **Step 4: Smoke `Tron mine doctor` on a non-mining dev box**
...
## Summary
- New `OpenTron.mining` subsystem with `MiningProvider` ABC + `MinerRegistry`
- `vllm-pearl` provider wrapping Pearl's Docker container
- Runtime sidecar (`~/.OpenTron/runtime/mining.json`) for engine ↔ mining handoff
- Telemetry: on-demand reads in v1; `MiningTelemetryCollector` shipped unwired for v1.x
- New CLI: `Tron mine init|start|stop|status|doctor|attach|logs`
...
- [ ] Manual smoke: `uv run Tron mine doctor` on a non-mining dev box prints honest output
