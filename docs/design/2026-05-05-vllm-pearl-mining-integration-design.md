...
### In scope (v1)
- New `OpenTron.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `TronConfig`
- New CLI namespace: `Tron mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.OpenTron/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.OpenTron/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
src/OpenTron/cli/
    mine_cmd.py          # Tron mine init|start|stop|status|doctor|attach|logs
...
`MinerRegistry` added to `src/OpenTron/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
[mining.extra]
docker_image_tag         = "OpenTron/pearl-miner:<pinned-ref>"
...
`~/.OpenTron/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
1. `Tron mine start` → `MinerRegistry.get("vllm-pearl").start(config)`.
...
4. `Tron ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `Tron mine start` and write the sidecar themselves via `Tron mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `Tron mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `Tron mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `Tron mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `Tron mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `Tron mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `Tron mine attach` | Manual mode: write sidecar without launching. |
| `Tron mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  OpenTron/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.OpenTron.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`Tron daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.OpenTron/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `Tron ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`Tron mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.OpenTron/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `Tron mine logs` post-mortem.
...
- `Tron mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `Tron mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `Tron mine prune` (v1.x) cleans old `OpenTron/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `Tron mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `Tron telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track OpenTron#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.OpenTron/runtime/mining.lock`; second invocation errors clearly | clear message |
