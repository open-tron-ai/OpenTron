...
!!! tip "Prerequisites"
    ...
    - OpenTron installed: run `uv sync --extra dev` from the repository root
...
The script creates a `Tron` instance and delegates the research task to an `OrchestratorAgent` with five tools wired in. The orchestrator iterates through multiple tool-calling turns, deciding at each step whether to search, store, think, or synthesize.

```mermaid
sequenceDiagram
    participant U as User
    participant J as Tron SDK
...
j = Tron(model="qwen3:8b", engine_key="ollama")  # (1)!
try:
    response = j.ask(
...
1. Creates a `Tron` instance targeting the local Ollama engine with `qwen3:8b`. Both parameters are optional — omitting them uses auto-detected defaults from `~/.OpenTron/config.toml`.
...
from OpenTron.recipes import load_recipe
...
This is useful when you want to version-control the research configuration, share it with collaborators, or feed it to the `Tron eval` runner for benchmarking.
...
Run `Tron agent info orchestrator` to see the full tool catalog.
...
- [Getting Started: Configuration](../getting-started/configuration.md) — how to configure engines and models in `~/.OpenTron/config.toml`
