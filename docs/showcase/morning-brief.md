...
# ☕ Morning Brief — Tron reads everything overnight so I don't have to
...
- one thing I asked Tron to track for me ("did Tuesday's deploy roll out cleanly?")
...
- **Nothing leaves my house.** My inbox, my Slack DMs, my calendar — Tron reads them locally and writes the digest locally. The only network call is the Discord webhook to my own private server.
- **It learns my taste.** Over a few weeks Tron figured out that PR titles starting with `chore:` aren't worth surfacing and that I don't want to see calendar holds I created myself. The summarizer has a `MEMORY.md` it updates when I react with 👎 to a bullet.
