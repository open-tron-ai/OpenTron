...
OpenTron ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the OpenTron team's PostHog instance —
...
[`src/OpenTron/analytics/events.py`](../src/OpenTron/analytics/events.py).
...
1. [`src/OpenTron/analytics/redaction.py`](../src/OpenTron/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/OpenTron/analytics/events.py`](../src/OpenTron/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the OpenTron leaderboard. It is **not** the same as analytics and
