---
title: Code Snippets
description: Copy-paste patterns for common OpenTron tasks
---

# Code Snippets

Ready-to-use patterns for the most common OpenTron tasks. Each snippet is self-contained and copy-pasteable.
...
Run with: `Tron compose run research_assistant "quantum computing advances"`

## API Server (1 command)

```bash
Tron serve --port 8000 --engine ollama --model qwen3:8b
```
...
```bash
docker build -t OpenTron .
docker run -p 8000:8000 OpenTron serve --host 0.0.0.0
```
