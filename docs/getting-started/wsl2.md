...
OpenTron on Windows installs two ways: **WSL2** (this page — the
...
About 3 minutes. Type `Tron` to start.
...
- The first time you run `Tron`, the WSL kernel may show a "process running in background" notification — that's the bg-orchestrator detaching. It's expected.
- Models are stored in WSL's filesystem (`~/.OpenTron/`), not your Windows drive. To free up space later: `Tron-uninstall` removes everything.
