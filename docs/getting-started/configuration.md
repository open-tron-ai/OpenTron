---
title: Configuration
description: Complete reference for OpenTron configuration
---

# Configuration

OpenTron uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
```
~/.OpenTron/config.toml
```

OpenTron creates the `~/.OpenTron/` directory and populates it with a default config when you run `Tron init`.

## Relocating the OpenTron directory

OpenTron keeps **all** of its state — config, databases, caches, logs,
...
`~/.OpenTron`, but you can move it.

The root is resolved in priority order:

1. **`$OpenTron_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/OpenTron`** — used when `$XDG_DATA_HOME` is set (a single
   `OpenTron` directory nested under it, per the XDG Base Directory spec).
3. **`~/.OpenTron`** — the default. With no environment variables set, the
   resolved path is exactly this, so existing installs are untouched.

```bash
# Relocate the whole install + runtime tree at install time:
OpenTron_HOME=~/apps/OpenTron curl -fsSL https://open-Tron.github.io/OpenTron/install.sh | bash

# Or for a single run / your shell profile:
export OpenTron_HOME=~/apps/OpenTron
```
...
!!! note "Migration"
    ...
    existing installs. If you set `OpenTron_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.OpenTron`, OpenTron will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.OpenTron "$OpenTron_HOME"`.

`$OpenTron_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.OpenTron/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `Tron init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, OpenTron automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, OpenTron uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
Tron ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.OpenTron/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `Tron serve`.
...
```bash
Tron serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.OpenTron/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.OpenTron/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.OpenTron/skills/` | Directory where skills are installed. |
...
overlay_dir = "~/.OpenTron/learning/skills/"
```
...
| `overlay_dir` | string | `~/.OpenTron/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `Tron init`, OpenTron probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.OpenTron/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure OpenTron entirely from Python without a TOML file:

```python
from OpenTron import Tron
from OpenTron.core.config import (
    AgentConfig,
    EngineConfig,
    IntelligenceConfig,
    TronConfig,
...
```python
j = Tron(config_path="/path/to/my-config.toml")
```
...
OpenTron respects the following environment variables:
