...
- The installer picks `mlx` as the recommended engine via the standard hardware-detect path, but the foreground default is still Ollama for compatibility. Switch later with `Tron init --force` and pick `mlx` if you've installed `mlx-lm`.
