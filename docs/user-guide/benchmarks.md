...
OpenTron ships with two benchmarks:
...
from OpenTron.bench._stubs import BenchmarkResult
...
from OpenTron.bench.latency import LatencyBenchmark
from OpenTron.bench.throughput import ThroughputBenchmark
...
Tron bench run -b throughput -n 20

# Specify model and engine
Tron bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
Tron bench run --json

# Write JSONL results to a file
Tron bench run -o results.jsonl

# Combine options
Tron bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from OpenTron.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
Tron bench run -b context_length
```
