...
from OpenTron.tools._stubs import ToolSpec
...
# Execute a tool call
from OpenTron.core.types import ToolCall
...
```python
from OpenTron.tools._stubs import build_tool_descriptions
...
```python
from OpenTron.tools.retrieval import RetrievalTool
from OpenTron.memory.sqlite import SQLiteMemory
...
```python
from OpenTron.tools.llm_tool import LLMTool
...
```python
from OpenTron.tools.file_read import FileReadTool
...
```python
from OpenTron.scheduler.tools import ScheduleTaskTool
from OpenTron.scheduler.scheduler import TaskScheduler
from OpenTron.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.OpenTron/scheduler.db")
scheduler = TaskScheduler(store=store, system=Tron_system)
scheduler.start()
...
```python
from OpenTron.core.registry import ToolRegistry
from OpenTron.tools._stubs import BaseTool, ToolSpec
from OpenTron.core.types import ToolResult
...
```bash
Tron ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
Tron ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
Tron ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
