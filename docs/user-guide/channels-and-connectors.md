...
OpenTron has two types of integrations:
...
```json
{
    "display_information": { "name": "OpenTron" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "OpenTron", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "OpenTron" |
...
Slack serves two purposes in OpenTron:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [OpenTron icon](https://github.com/open-Tron/OpenTron/blob/main/assets/OpenTron-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **OpenTron** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "OpenTron"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @OpenTron` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @OpenTron` in the channel |
...
3. **Connect in OpenTron:**
   ...
   - CLI: `uv run Tron connect notion`
...
3. **Connect in OpenTron:**
   ...
   - CLI: `uv run Tron connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the OpenTron desktop app
...
3. **Connect in OpenTron:**
   ...
   - CLI: `uv run Tron connect outlook`
...
2. **Connect in OpenTron:**
   - Desktop/Browser: Agents → Channels tab → Obsidian → paste the vault path
   - CLI: `uv run Tron connect obsidian --path /path/to/vault`
...
4. **Connect in OpenTron:**
   ...
   - CLI: `uv run Tron connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run Tron deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.OpenTron/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.OpenTron/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
