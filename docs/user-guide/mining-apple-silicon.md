...
README. Then write OpenTron' mining config:
...
```bash
Tron mine doctor        # capability matrix
Tron mine start         # launch gateway + miner-loop subprocesses
Tron mine status        # check sidecar + gateway metrics
Tron mine logs -n 120   # print recent logs
Tron mine stop          # stop mining subprocesses
```
