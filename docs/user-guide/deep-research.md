...
This writes a pre-configured `~/.OpenTron/config.toml` for the deep research agent.
...
OpenTron chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
Tron ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
Tron ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
Tron memory index ~/Downloads/reports/
Tron memory index ./notes.md

# Search memory directly
Tron memory search "project timeline"
Tron memory search -k 20 "budget estimates"
...
Tron ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
Tron ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
Tron ask "What meetings did I have with Alice last month?"

# Extract structured data
Tron ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
Tron ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
Tron memory index ./report.pdf

# Custom chunk size for long documents
Tron memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
curl -s https://example.com/article | Tron memory index --stdin --source "example.com"
```

### Code repositories

```bash
Tron memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `Tron memory index`. Check indexed content with `Tron memory stats`.
