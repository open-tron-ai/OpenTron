...
The channels module lets OpenTron send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from OpenTron.channels.telegram import TelegramChannel
...
from OpenTron.channels._stubs import ChannelMessage
from OpenTron.channels.discord_channel import DiscordChannel
...
from OpenTron.channels.slack import SlackChannel
...
from OpenTron.core.events import EventBus, EventType
from OpenTron.channels.telegram import TelegramChannel
...
The `Tron channel` subcommand group provides quick access to channel operations.
...
Tron channel send telegram "Build completed successfully"
```
...
When `Tron serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.OpenTron/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.OpenTron/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from OpenTron!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  OpenTron
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. OpenTron sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since OpenTron runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from OpenTron.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from OpenTron!")
```
...
Tron channel status --channel-type sendblue

# Send a message
Tron channel send sendblue "+15551234567" "Hello from Tron!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `Tron serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.OpenTron/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from OpenTron.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Tron",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.OpenTron/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.OpenTron/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Tron"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.OpenTron/whatsapp_baileys_bridge/auth"
assistant_name = "Tron"
...
- [API Reference: Channels](../api-reference/OpenTron/channels/index.md) -- full class and type signatures
