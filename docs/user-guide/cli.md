...
OpenTron provides a command-line interface through the `Tron` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
Tron --version   # Print the OpenTron version
Tron --help      # Show top-level help with all subcommands
```

## `Tron init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.OpenTron/config.toml`.

```bash
Tron init           # Interactive — refuses to overwrite existing config
Tron init --force   # Overwrite existing config without prompting
```
...
```bash
Tron ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
Tron ask --agent orchestrator "What is 2+2?"
Tron ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
Tron ask --agent simple "Hello"
```
...
# Specify a model
Tron ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
Tron ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
Tron ask --json "Hello"

# Disable memory context injection
Tron ask --no-context "Tell me about Python"

# Set maximum token generation
Tron ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
Tron ask -i screenshot.png "What is shown in this image?"

# Send multiple images (the flag is repeatable)
Tron ask -i chart-a.png -i chart-b.png "Compare these two charts"

# Capture the current screen and ask about it
Tron ask --screen "Summarize what's on my screen"
```
...
the `Tron_NUM_CTX` environment variable (default `16384`):

```bash
Tron_NUM_CTX=8192 Tron ask --screen "What's on my screen?"
```

!!! note "Keep vision on-device"
    Images are sensitive. OpenTron prints a privacy warning before sending
...
Access Pearl's native node, wallet, and RPC tools from the OpenTron CLI.

```bash
Tron pearl doctor
Tron pearl node -- <pearld args>
Tron pearl wallet -- <oyster args>
Tron pearl ctl -- <prlctl args>
Tron pearl address
```

All Pearl wrapper commands use the `Tron pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| OpenTron command | Pearl binary | Use |
|--------------------|--------------|-----|
| `Tron pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `Tron pearl node` | `pearld` | Run the Pearl full node |
| `Tron pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `Tron pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `Tron pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `Tron memory index <path>`

Index documents from a file or directory into the memory store.

```bash
Tron memory index ./docs/
Tron memory index ./notes.md
Tron memory index ./data/ --chunk-size 256 --chunk-overlap 32
Tron memory index ./docs/ --backend sqlite
```
...
### `Tron memory search <query>`

Search the memory store for relevant document chunks.

```bash
Tron memory search "machine learning basics"
Tron memory search -k 10 "neural networks"
Tron memory search --backend faiss "embeddings"
```
...
```bash
Tron memory stats
Tron memory stats --backend sqlite
```
...
```bash
Tron telemetry stats
Tron telemetry stats -n 5    # Show top 5 models
```
...
### `Tron telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
Tron telemetry export                          # JSON to stdout
Tron telemetry export --format csv             # CSV to stdout
Tron telemetry export --format json -o data.json  # JSON to file
Tron telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `Tron telemetry clear`

Delete all telemetry records from the database.

```bash
Tron telemetry clear         # Interactive confirmation
Tron telemetry clear --yes   # Skip confirmation
```
...
```bash
Tron bench run                               # Run all benchmarks, 10 samples
Tron bench run -n 20                         # 20 samples per benchmark
Tron bench run -b latency                    # Only the latency benchmark
Tron bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
Tron bench run -o results.jsonl              # Write JSONL results to file
Tron bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
```bash
Tron channel send slack "Hello from Tron!"
Tron channel send discord "Build complete"
```
...
```bash
Tron serve                                 # Default host/port from config
Tron serve --port 8000                     # Custom port
Tron serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
Tron serve --model qwen3:8b                # Specify default model
Tron serve --agent orchestrator            # Route requests through an agent
```
...
`Tron` subcommand for it. Construct a `SpecSearchOrchestrator`
directly from `OpenTron.learning.spec_search.orchestrator` and call
`.run(trigger)` with a trigger from
`OpenTron.learning.spec_search.triggers`. See
