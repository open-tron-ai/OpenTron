...
OpenTron has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.OpenTron/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.OpenTron/telemetry.db")
...
Tron telemetry stats -n 5          # Top 5 models only

# Export records
Tron telemetry export              # JSON to stdout
Tron telemetry export -f csv       # CSV to stdout
Tron telemetry export -o data.json # JSON to file
Tron telemetry export -f csv -o metrics.csv

# Clear all records
Tron telemetry clear               # With confirmation prompt
Tron telemetry clear --yes         # Without confirmation
```
