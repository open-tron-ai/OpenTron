...
The OpenTron evaluation framework (`OpenTron.evals`) measures model **correctness and accuracy** on academic datasets. It ships inside the main `OpenTron` package (at `src/OpenTron/evals/`) and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    OpenTron has two distinct measurement systems that complement each other:

    | System | Module | Measures | Entry Point |
    |--------|--------|----------|-------------|
    | **Evaluations** | `OpenTron.evals` | Correctness on academic datasets (accuracy, pass rate) | `Tron eval` |
    | **Benchmarks** | `OpenTron.bench` | Engine performance (latency, throughput) | `Tron bench` |
...
The evaluation framework is part of the main `OpenTron` package — no separate install or extra is required. The standard dev setup is enough:

```bash
uv sync --extra dev
```

The framework's core dependencies (`click`, `datasets`, `rich`) are base dependencies of `OpenTron`. Two optional extras enable experiment tracking integrations:
...
!!! note "Python version requirement"
    Python 3.10 requires the `tomli` package for TOML config parsing. `OpenTron` declares it as a conditional dependency, so it is installed automatically.
...
| Command | Surface |
|---------|---------|
| `Tron eval {list,run,compare,report}` | Canonical CLI. `run` covers the common options; `compare` and `report` post-process result files. |
| `python -m OpenTron.evals {list,run,run-all,summarize,reparse-judge}` | Full research surface, including judge configuration, the agentic runner, and episode mode. |

The `OpenTron-eval` console script is an alias for `python -m OpenTron.evals` — same commands, same options. This guide uses `Tron eval` wherever its option set suffices and the module form for research-only options.

---

## Datasets

The framework ships with **40 registered benchmarks** covering academic reasoning, agentic tasks, coding, retrieval, conversation quality, and practical use-case benchmarks. Datasets are grouped by category below; `uv run python -m OpenTron.evals list` prints the authoritative registry.

### Use-Case Benchmarks

These benchmarks evaluate models on practical tasks that mirror real OpenTron use cases.
...
```bash
uv run Tron eval run --config src/OpenTron/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run Tron eval run --config src/OpenTron/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **Tron-direct** | `Tron-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **Tron-agent** | `Tron-agent` | Agent-level inference with tool calling. Uses `TronSystem.ask()` with the specified agent and tools. |
...
Use `Tron-direct` for most evaluations. Use `Tron-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.

The `hermes` and `openclaw` backends shell out to external agent frameworks and need an OpenAI-compatible endpoint for their model calls: pass `--base-url`/`--api-key`, set the `Tron_BACKEND_BASE_URL`/`Tron_BACKEND_API_KEY` environment variables, or add a `[backend.external]` section to your config (see [Config Reference](#backendexternal)).

!!! note "TerminalBench Native"
    `Tron eval run --backend` additionally accepts `terminalbench-native`, a Docker-based execution backend used by the TerminalBench Native benchmark.
...
```bash
uv run python -m OpenTron.evals list
```
...
└──────────────────────┴───────────┴───────────────────────────────────┘
                         Available Backends
┌───────────────┬──────────────────────────────────────────────────┐
│ Tron-direct │ Engine-level inference (local or cloud)          │
│ Tron-agent  │ Agent-level inference with tool calling          │
│ hermes        │ Real Hermes Agent (Nous Research) via subprocess │
│ openclaw      │ Real OpenClaw via Node subprocess                │
└───────────────┴──────────────────────────────────────────────────┘
```

`Tron eval list` prints a similar table but currently shows a curated subset of the registry; the module form above is the authoritative listing.

### Run a single benchmark

```bash
# Evaluate qwen3:8b on SuperGPQA (engine-level, 10 samples)
uv run Tron eval run -b supergpqa -m qwen3:8b -n 10

# Evaluate GPT-5 Mini on GAIA using the agent backend with tools
uv run Tron eval run -b gaia -m gpt-5-mini --backend Tron-agent \
    --agent orchestrator --tools calculator,file_read -n 50

# Run FRAMES with the vLLM engine, write output to a file
uv run Tron eval run -b frames -m llama3:70b -e vllm \
    -o results/frames_llama70b.jsonl

# Run WildChat with a higher temperature for chat quality
uv run Tron eval run -b wildchat -m qwen3:8b --temperature 0.7 -n 100
```

#### `Tron eval run` option reference
...
| `--backend` | | choice | `Tron-direct` | `Tron-direct`, `Tron-agent`, `hermes`, `openclaw`, or `terminalbench-native` |
| `--base-url` | | str | — | OpenAI-compatible endpoint URL (env: `Tron_BACKEND_BASE_URL`) |
| `--api-key` | | str | — | API key for the endpoint (env: `Tron_BACKEND_API_KEY`) |
| `--agent` | | str | — | Agent name for `Tron-agent` backend (e.g., `orchestrator`) |
...
#### Research-only options (`python -m OpenTron.evals run`)

The module CLI accepts everything above plus research-grade options that `Tron eval run` does not expose:
...
Note: the module CLI's `--backend` choice covers `Tron-direct`, `Tron-agent`, `hermes`, and `openclaw`; `terminalbench-native` as a backend is available via `Tron eval run` and TOML configs.
...
```bash
uv run python -m OpenTron.evals run-all -m qwen3:8b

# With options
uv run python -m OpenTron.evals run-all -m gpt-5-mini -n 100 --output-dir results/gpt5mini/
```
...
```bash
uv run python -m OpenTron.evals summarize results/supergpqa_qwen3-8b.jsonl
```
...
`Tron eval` adds two post-processing commands for result files:

```bash
# Side-by-side metric comparison across runs
uv run Tron eval compare results/supergpqa_qwen3-8b.jsonl results/supergpqa_gpt-5-mini.jsonl

# Detailed report (accuracy, latency, cost, per-subject breakdown) for one run
uv run Tron eval report results/supergpqa_qwen3-8b.jsonl
```
...
If you already have an OpenAI-compatible server running — `Tron serve`, vLLM, SGLang, llama.cpp's server, or a hosted endpoint — point an eval directly at it with `--base-url` and `--api-key`:

```bash
# A vLLM server is already serving Qwen/Qwen3-8B on a GPU node:
#   vllm serve Qwen/Qwen3-8B --port 8000
uv run Tron eval run -b supergpqa -m Qwen/Qwen3-8B \
    --base-url http://gpu-node:8000/v1 \
    --api-key local-key \
    -n 50
```

The `-m` value must match a model id the server reports at `GET /v1/models`. Both flags fall back to the `Tron_BACKEND_BASE_URL` and `Tron_BACKEND_API_KEY` environment variables, so CI jobs can set them once:

```bash
export Tron_BACKEND_BASE_URL=http://gpu-node:8000/v1
export Tron_BACKEND_API_KEY=local-key
uv run Tron eval run -b gaia -m Qwen/Qwen3-8B --backend Tron-agent -n 25
```
...
!!! tip "Engine-level alternative for vLLM"
    ...
    ```bash
    VLLM_HOST=http://gpu-node:8000 uv run python -m OpenTron.evals run \
...
```bash
uv run Tron eval run --config src/OpenTron/evals/configs/full-suite.toml
```
...
```toml title="src/OpenTron/evals/configs/full-suite.toml"
...
```toml title="src/OpenTron/evals/configs/minimal.toml"
[[models]]
name = "qwen3:8b"
...
```toml title="src/OpenTron/evals/configs/single-run.toml"
[meta]
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `base_url` | str | `None` | OpenAI-compatible endpoint URL (env: `Tron_BACKEND_BASE_URL`) |
| `api_key` | str | `None` | API key for the endpoint (env: `Tron_BACKEND_API_KEY`) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | str | required | Any registered benchmark key (see `uv run python -m OpenTron.evals list`) |
| `backend` | str | `"Tron-direct"` | `Tron-direct`, `Tron-agent`, `hermes`, `openclaw`, or `terminalbench-native` |
...
| `agent` | str | `None` | Agent name for `Tron-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `Tron-agent` backend |
...
uv run python -m OpenTron.evals run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `Tron-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to OpenTron inference and agents
