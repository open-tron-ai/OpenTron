...
| **Overlay** | Sidecar file at `~/.OpenTron/learning/skills/` storing optimized descriptions and few-shot examples |
...
Tron skill install hermes:apple-notes

# Bulk install a category
Tron skill sync hermes --category research

# Run a skill directly
Tron skill run math-solver -a expression="41 + 82"

# See skill details
Tron skill info research-and-summarize
```
...
Tron skill sync hermes --category research
Tron skill sync hermes --category coding
Tron skill sync hermes  # everything (~150 skills)
```
...
Tron skill install openclaw:0xv4l3nt1n3/etherscan
...
```bash
Tron skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
Tron skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.OpenTron/config.toml` for automatic syncing:
...
OpenTron can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
Tron skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.OpenTron/skills/discovered/
Tron skill discover
```

Discovered skills land in `~/.OpenTron/skills/discovered/` and automatically appear in `Tron skill list` on the next session.
...
Tron optimize skills --dry-run

# Run DSPy optimization
Tron optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
Tron optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
Tron skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.OpenTron/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
Tron bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with OpenTron | Implicit trust | Full access within declared capabilities |
...
```bash
Tron skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.OpenTron/skills/"  # where skills are installed
...
2. **User** `~/.OpenTron/skills/`
3. **Bundled** (shipped with OpenTron)

## CLI Reference

| Command | Description |
|---------|-------------|
| `Tron skill list` | List installed skills |
| `Tron skill info <name>` | Show detailed skill information |
| `Tron skill run <name> [-a key=value]` | Execute a skill directly |
| `Tron skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `Tron skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `Tron skill sources` | List configured skill sources |
| `Tron skill update` | Pull latest from configured sources |
| `Tron skill remove <name>` | Remove an installed skill |
| `Tron skill search <query>` | Search the skill index |
| `Tron skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `Tron skill show-overlay <name>` | Inspect optimization output for a skill |
| `Tron optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `Tron bench skills [--condition C]` | Run the PinchBench skills benchmark |
