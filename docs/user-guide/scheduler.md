...
!!! note "Optional component"
    The scheduler is a standalone module (`OpenTron.scheduler`). It is not wired into the default `Tron` / `TronSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `Tron scheduler start`.
...
The `Tron scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
Tron scheduler list --status active

# Paused tasks
Tron scheduler list --status paused
```
...
Tron scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
Tron scheduler resume a3f9b12c4d8e
```
...
Tron scheduler cancel a3f9b12c4d8e
```
...
Tron scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.OpenTron/scheduler.db")  # (1)!

# Wire in a TronSystem for task execution
...
4. Starts a daemon thread named `"Tron-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.OpenTron/config.toml`.

```toml title="~/.OpenTron/config.toml"
[scheduler]
enabled = false
db_path = "~/.OpenTron/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.OpenTron/scheduler.db`     | SQLite database path                       |
