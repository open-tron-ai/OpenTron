...
Agents are the agentic logic layer of OpenTron. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
from OpenTron.agents._stubs import AgentContext, AgentResult
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from OpenTron.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
Tron ask --agent simple "What is the capital of France?"

# Orchestrator with tools
Tron ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
Tron ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
Tron ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
Tron ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
Tron ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
Tron ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.OpenTron/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.OpenTron/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
4. The Python side parses the output between `---OpenTron_OUTPUT_START---` and `---OpenTron_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what OpenTron tool-calling agents support.

```python
from OpenTron.agents.claude_code import ClaudeCodeAgent
...
```bash
# Via CLI
Tron ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept OpenTron tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; OpenTron supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your OpenTron engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`OpenTron/<model>`).
...
| `provider_id`       | `str`             | `"OpenTron"`   | opencode provider id to register/use                     |
...
```python
from OpenTron.agents.opencode import OpenCodeAgent
...
Tron ask --agent opencode "Refactor the parser to use a state machine"
```
...
```python
from OpenTron.agents.operative import OperativeAgent
...
```bash
# Via CLI
Tron ask --agent operative "Check system status"
```
...
```python
from OpenTron.agents.monitor_operative import MonitorOperativeAgent
...
```bash
# Via CLI
Tron ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from OpenTron.sandbox import ContainerRunner, SandboxedAgent
from OpenTron.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="OpenTron-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/OpenTron/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"OpenTron-sandbox:latest"`| Docker image to run                            |
...
```python
from OpenTron.core.registry import AgentRegistry
