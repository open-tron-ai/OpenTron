...
This writes a pre-configured `~/.OpenTron/config.toml` for the operative agent with scheduling support.
...
Tron scheduler status <task-id>

# Pause / resume / delete a task
Tron scheduler pause <task-id>
Tron scheduler resume <task-id>
Tron scheduler delete <task-id>

# Run a task immediately (outside its schedule)
Tron scheduler run <task-id>
...
- **All data stays local**: State is stored in `~/.OpenTron/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `Tron scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `Tron scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `Tron memory stats`. Clear old entries with `Tron memory clear --before 2026-01-01` if needed.
