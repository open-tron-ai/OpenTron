...
```bash
python examples/OpenTron/spec_search_quickstart.py
```
...
`OpenTron.learning.spec_search.composite_reward.score_batch`.
...
`configs/OpenTron/examples/spec-search-quickstart.toml`. Copy it to
`~/.OpenTron/config.toml` (or set `OpenTron_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `OpenTron.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `OpenTron.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `OpenTron.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `OpenTron.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `OpenTron.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/OpenTron/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `OpenTron.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from OpenTron.evals.datasets.adp import ADPDataset
from OpenTron.learning.spec_search.external_adapter import (
    write_external_records_as_traces,
)
from OpenTron.traces.store import TraceStore

records = list(ADPDataset().load(max_samples=200, seed=42, split="all"))
store = TraceStore("~/.OpenTron/traces.db")
...
`src/OpenTron/evals/backends/Tron_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.OpenTron/traces.db` regardless of caller
...
- `examples/OpenTron/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/OpenTron/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/OpenTron/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/OpenTron/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/OpenTron/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/OpenTron/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/OpenTron/learning/spec_search/external_adapter.py` — corpus → trace adapter.
