...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.OpenTron/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.OpenTron/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.OpenTron/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." Tron digest --fresh
```
...
```bash
Tron digest --fresh          # Generate a new digest now
Tron digest                  # Show today's cached digest
Tron digest --text-only      # Print text without audio
Tron digest --history        # Show past digests
Tron digest --schedule "0 6 * * *"   # Set daily schedule
Tron digest --schedule off   # Disable schedule
Tron digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Tron (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/OpenTron/prompts/personas/{name}.md`. The default `Tron` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.OpenTron/connectors/news_rss.json`:
...
**"No digest for today"** — Run `Tron digest --fresh` to generate one.

**Empty sections** — Check connector status with `Tron connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
