...
This writes a pre-configured `~/.OpenTron/config.toml` for the code assistant.
...
```bash
Tron ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
Tron ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
Tron ask "Read main.py and explain the architecture"

# Fix a bug
Tron ask "Find and fix the bug in test_utils.py"

# Run tests
Tron ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
Tron ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.OpenTron/config.toml`:
...
```bash
# Write a new script
Tron ask "Write a Python script that converts YAML to JSON"

# Explain existing code
Tron ask "Read src/OpenTron/core/events.py and explain the EventBus pattern"

# Debug a failing test
Tron ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
Tron ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
Tron ask "Read src/OpenTron/tools/calculator.py and write unit tests for it"

# Shell tasks
Tron ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`Tron chat`).
- For stronger isolation, use the sandboxed agent: `Tron ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `Tron`. Use `cd /path && command` in your prompt if needed, or run `Tron` from the project directory.
