...
```python
from OpenTron.core.registry import ModelRegistry
...
```python
from OpenTron.intelligence import register_builtin_models
...
```python
from OpenTron.intelligence import merge_discovered_models
...
```python
from OpenTron.intelligence import (
...
The following names are still importable from `OpenTron.intelligence` via shim modules, but their canonical locations have moved:
