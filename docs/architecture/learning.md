...
!!! note "Backward compatibility"
    The canonical locations are now `OpenTron.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `OpenTron.core.types` (for `RoutingContext`). The old `OpenTron.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `OpenTron.learning._stubs`.
...
```bash
Tron ask --router learned "What is the capital of France?"
```
...
```python
from OpenTron.learning.router import HeuristicRouter, build_routing_context
...
```python
from OpenTron.learning.router import build_routing_context
...
```python
from OpenTron.learning.trace_policy import TraceDrivenPolicy
from OpenTron.traces.analyzer import TraceAnalyzer
...
```python
from OpenTron.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from OpenTron.learning.sft_policy import SFTPolicy
```
...
```python
from OpenTron.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from OpenTron.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from OpenTron.learning.heuristic_reward import HeuristicRewardFunction
...
```python
from OpenTron.traces.collector import TraceCollector
...
over OpenTron's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `OpenTron-learning`
