...
description: The five-primitive architecture behind OpenTron — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
All extensible components in OpenTron use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/OpenTron/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           TronConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              Tron ask -- query the assistant
        serve.py            Tron serve -- start API server
