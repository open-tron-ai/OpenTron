...
- **Persistence:** Data persists across restarts in `~/.OpenTron/memory.db`
...
from OpenTron.tools.storage.faiss_backend import FAISSMemory
from OpenTron.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from OpenTron.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `OpenTron.tools.storage.*`.
...
```python
from OpenTron.tools.storage.chunking import chunk_text, ChunkConfig
...
from OpenTron.tools.storage.ingest import ingest_path
...
```python
from OpenTron.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from OpenTron.tools.storage.context import inject_context, ContextConfig
...
from OpenTron.tools.storage._stubs import MemoryBackend
...
The default backend is configured in `~/.OpenTron/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
