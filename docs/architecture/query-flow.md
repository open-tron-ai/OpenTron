...
This page traces the end-to-end journey of a user query through the OpenTron system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: Tron ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: TronConfig (hardware, engine defaults)
...
OpenTron supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
Tron ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.OpenTron/config.toml
```
...
- Returns a `TronConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.OpenTron/telemetry.db`.
...
The `Tron` class in `sdk.py` provides the same query flow through a Python API:

```python
from OpenTron import Tron

j = Tron(model="qwen3:8b", engine_key="ollama")
