...
OpenTron follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in OpenTron is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
from OpenTron.engine._stubs import InferenceEngine
...
OpenTron is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Tron(engine_key="ollama")  # Local Ollama server
...
OpenTron **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `Tron init` and used as the default engine:

```bash
Tron init --force
# Detects hardware, writes ~/.OpenTron/config.toml with:
# [engine]
...
```bash
Tron telemetry stats          # Aggregated statistics
Tron telemetry export --json  # Export all records
```
...
OpenTron provides a **clean Python API** through the `Tron` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`OpenTron[server]`, `OpenTron[memory-colbert]`, etc.)
...
The API server (`Tron serve`) implements the **OpenAI chat completions API format**, making OpenTron a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to OpenTron:
...
OpenTron requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of OpenTron.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `OpenTron[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `OpenTron[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `OpenTron[memory-bm25]` | `rank-bm25` |
| API server | `OpenTron[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `OpenTron[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `OpenTron[inference-vllm]` | `vllm` |
| PDF ingestion | `OpenTron[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `OpenTron[channel-whatsapp-baileys]` | Node.js 22+ |
