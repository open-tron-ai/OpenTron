...
OpenTron supports [ACE](https://github.com/ace-agent/ace) as a third
...
ACE is **not on PyPI** as of OpenTron v1.0.1, and the upstream
...
```bash
# 1. Clone ACE somewhere outside your OpenTron checkout
git clone https://github.com/ace-agent/ace.git ~/code/ace
cd ~/code/ace
curl -LsSf https://astral.sh/uv/install.sh | sh   # if you don't have uv
uv sync

# 2. Make ACE's src/ importable from your OpenTron venv
...
# 4. Verify the import resolves from OpenTron's venv
python -c "from OpenTron.learning.agents.ace_optimizer import HAS_ACE; print(HAS_ACE)"
# True
```

If `HAS_ACE` prints `False`, the `.pth` file isn't being picked up —
verify the path matches `site.getsitepackages()[0]` for the same
Python interpreter you're using to run OpenTron.

## Configuration

ACE is configured under `[learning.agent.ace]` in your OpenTron
config TOML:
...
task_name = "OpenTron"
save_dir = ""               # default: ~/.OpenTron/learning/ace/<task>/
...
```bash
Tron optimize agent --policy ace
```

ACE writes intermediate state and the final playbook to `save_dir`.
The OpenTron runtime will pick up the playbook on next agent start
(via the same sidecar overlay mechanism the Skills System uses).

## Trace adapter behavior

OpenTron traces are adapted into ACE's `train_samples` /
...
in `src/OpenTron/learning/agents/ace_optimizer.py` — it does a
...
- **Single provider per run.** ACE assigns the same `api_provider` to
  all three roles. To mix providers, run ACE outside OpenTron and
