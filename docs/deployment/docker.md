...
OpenTron provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.
...
cd deploy/docker
cp .env.example .env
echo "OpenTron_API_KEY=$(Tron auth generate-key)" > .env   # or paste your own
```
...
`docker compose` reads `OpenTron_API_KEY` from `.env` (or your shell
...
1. **Builder stage** -- installs `uv` and the `OpenTron[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker build -t OpenTron:latest .
```

Run it standalone:

```bash
docker run -d -p 8000:8000 OpenTron:latest
```
...
```bash
docker build -f Dockerfile.gpu -t OpenTron:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 OpenTron:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the OpenTron API server and an Ollama backend:
...
services:
  Tron:
    ...
    ports:
      - "8000:8000"
    environment:
      - OpenTron_ENGINE_DEFAULT=ollama
      - OpenTron_OLLAMA_HOST=http://ollama:11434
    depends_on:
      - ollama
...
The `Tron` service is configured through environment variables:

| Variable                      | Description                                             | Default                    |
|-------------------------------|---------------------------------------------------------|----------------------------|
| `OpenTron_ENGINE_DEFAULT`   | Inference engine backend to use                         | `ollama`                   |
| `OpenTron_OLLAMA_HOST`      | URL of the Ollama server (uses Docker service name)     | `http://ollama:11434`      |
...
The `Tron` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.OpenTron/config.toml`, which is `/root/.OpenTron/config.toml` in the container):
...
services:
  Tron:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.OpenTron/config.toml:ro
    environment:
      - OpenTron_ENGINE_DEFAULT=ollama
      - OpenTron_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire OpenTron data directory:
...
The entrypoint is `Tron` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 OpenTron:latest \
...
### Available CLI Options for `Tron serve`
