...
OpenTron includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.OpenTron/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
!!! warning "Server dependency check"
    If the `[server]` extra is not installed, `Tron serve` exits with a clear error message explaining how to install the required dependencies.
...
The `[server]` section of `~/.OpenTron/config.toml` controls default server behavior:
...
CLI flags override config file values. For example, `Tron serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run OpenTron behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    ...
    ssl_certificate /etc/ssl/certs/Tron.pem;
...
```bash
Tron serve --host 127.0.0.1 --port 8000
```
