/**
 * deep-link.ts — Handler for OpenTron:// deep links in the Tauri desktop app.
 *
 ...
 *   OpenTron://research/{session_id}   → open a research session
 *   OpenTron://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `OpenTron://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `OpenTron://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `OpenTron:` scheme.
 *
 * @example
 * parseDeepLink("OpenTron://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("OpenTron://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "OpenTron:") return null;

    // The URL constructor treats "OpenTron://research/abc123" such that
