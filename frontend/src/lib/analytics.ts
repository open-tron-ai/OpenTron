...
/**
 * Mirror of the Python event catalog (src/OpenTron/analytics/events.py
    ...
    if (import.meta.env.DEV) {
      console.warn(
        `[analytics] Unknown event "${event}" dropped. ` +
          ...
          `REGISTRY in src/OpenTron/analytics/events.py.`,
      );
    }
    return;
  }
 ...
 * :func:`OpenTron.analytics.redaction.hash_id`.
 */
