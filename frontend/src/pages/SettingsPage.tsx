...
export function SettingsPage() {
  ...
  const [memoryEnabled, setMemoryEnabled] = useState(() => {
    try { return localStorage.getItem('OpenTron-memory-enabled') !== 'false'; } catch { return true; }
  });
  const [memoryBackend, setMemoryBackend] = useState(() => {
    try { return localStorage.getItem('OpenTron-memory-backend') || 'sqlite'; } catch { return 'sqlite'; }
  });
  const [memoryTopK, setMemoryTopK] = useState(() => {
    try { return parseInt(localStorage.getItem('OpenTron-memory-top-k') || '5'); } catch { return 5; }
  });
  const [memoryMinScore, setMemoryMinScore] = useState(() => {
    try { return parseFloat(localStorage.getItem('OpenTron-memory-min-score') || '0.1'); } catch { return 0.1; }
  });
  const [memoryMaxTokens, setMemoryMaxTokens] = useState(() => {
    try { return parseInt(localStorage.getItem('OpenTron-memory-max-tokens') || '2048'); } catch { return 2048; }
  });

  const [speechBackend, setSpeechBackend] = useState(() => {
    try { return localStorage.getItem('OpenTron-speech-backend') || 'none'; } catch { return 'none'; }
  });
  const [speechOpenAIKey, setSpeechOpenAIKey] = useState(() => {
    try { return localStorage.getItem('OpenTron-speech-openai-key') || ''; } catch { return ''; }
  });
  const [speechKeySaved, setSpeechKeySaved] = useState(false);  
  const saveSpeechConfig = useCallback(async () => {
    try {
      // Save to localStorage for frontend
      localStorage.setItem('OpenTron-speech-backend', speechBackend);
      localStorage.setItem('OpenTron-speech-openai-key', speechOpenAIKey);
      
      // Save to disk via Tauri for backend (if running in Tauri)
      if ((window as any).__TAURI_INTERNALS__) {
        await invoke('save_speech_config_cmd', {
          backend: speechBackend,
          openaiKey: speechOpenAIKey,
        });
      }
      setSpeechKeySaved(true);
      setTimeout(() => setSpeechKeySaved(false), 2000);
    } catch (e) {
      console.error('Failed to save speech config:', e);
    }
  }, [speechBackend, speechOpenAIKey]);
  ...
  const handleExport = () => {
    const data = localStorage.getItem('OpenTron-conversations') || '{}';
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `OpenTron-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) {
            localStorage.setItem('OpenTron-conversations', JSON.stringify(data));
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="OpenAI" storageKey="OpenTron-openai-key" />
                <CloudProviderStatus label="Anthropic" storageKey="OpenTron-anthropic-key" />
                <CloudProviderStatus label="Google" storageKey="OpenTron-gemini-key" />
                <CloudProviderStatus label="OpenRouter" storageKey="OpenTron-openrouter-key" />
              </div>
            </SettingRow>
          </Section>
          ...
          <Section title="API Keys">
            <SettingRow label="OpenAI" description="GPT-4, GPT-3.5, etc.">
              <ApiKeyInput storageKey="OpenTron-openai-key" placeholder="sk-..." />
            </SettingRow>
            <SettingRow label="Anthropic" description="Claude models">
              <ApiKeyInput storageKey="OpenTron-anthropic-key" placeholder="sk-ant-..." />
            </SettingRow>
            <SettingRow label="Google" description="Gemini models">
              <ApiKeyInput storageKey="OpenTron-gemini-key" placeholder="AI..." />
            </SettingRow>
            <SettingRow label="OpenRouter" description="Multi-provider routing">
              <ApiKeyInput storageKey="OpenTron-openrouter-key" placeholder="sk-or-..." />
            </SettingRow>
          </Section>
            ...
            <SettingRow label="Use memory context" description="Automatically inject relevant memories into conversations">
              <button
                onClick={() => {
                  ...
                  try { localStorage.setItem('OpenTron-memory-enabled', String(next)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Memory backend" description="Which retrieval engine to use">
              <select
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('OpenTron-memory-backend', e.target.value); } catch {}
                  showSaved();
                }}
                className="text-sm px-3 py-1.5 rounded-lg outline-none cursor-pointer"
                style={{
            ...
            <SettingRow label="Results to inject" description={`${memoryTopK}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryTopK(v);
                  try { localStorage.setItem('OpenTron-memory-top-k', String(v)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Min relevance score" description={`${memoryMinScore}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryMinScore(v);
                  try { localStorage.setItem('OpenTron-memory-min-score', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
            ...
            <SettingRow label="Max context tokens" description={`${memoryMaxTokens}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('OpenTron-memory-max-tokens', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
          ...
          <Section title="Speech">
            ...
            <SettingRow label="Backend" description="Speech transcription engine">
              <div className="flex items-center gap-2">
                <select
                  value={speechBackend}
                  onChange={(e) => {
                    setSpeechBackend(e.target.value);
                    setSpeechKeySaved(false);
                  }}
                  className="text-sm px-3 py-1.5 rounded-lg outline-none cursor-pointer w-56"
                  style={{
                    background: 'var(--color-bg-secondary)',
                    color: 'var(--color-text)',
                    border: '1px solid var(--color-border)',
            ...
            {speechBackend === 'openai' && (
              <SettingRow label="OpenAI API Key" description="For Whisper speech transcription">
                <div className="flex items-center gap-2">
                  <input
                    type="password"
                    value={speechOpenAIKey}
                    onChange={(e) => {
                      setSpeechOpenAIKey(e.target.value);
                      setSpeechKeySaved(false);
                    }}
                    placeholder="sk-..."
                    className="w-48 px-2 py-1 rounded text-xs"
                  ...
                  <button
                    onClick={() => {
                      try { if (speechOpenAIKey) localStorage.setItem('OpenTron-speech-openai-key', speechOpenAIKey); else localStorage.removeItem('OpenTron-speech-openai-key'); } catch {}
                      setSpeechKeySaved(true);
                      setTimeout(() => setSpeechKeySaved(false), 2000);
                    }}
                    className="px-2 py-1 rounded text-xs font-medium cursor-pointer"
                    style={{ background: 'var(--color-accent, var(--color-bg-tertiary))', color: 'white', border: 'none' }}
                  >
                    Save
                  </button>
                  {speechKeySaved && <span className="text-[10px]" style={{ color: 'var(--color-success)' }}>Saved</span>}
                </div>
              </SettingRow>
            )}
            <SettingRow label="Backend status" description="Current speech transcription status">
              <div className="flex items-center gap-2">
                <span
                  className="w-2 h-2 rounded-full"
                  style={{
