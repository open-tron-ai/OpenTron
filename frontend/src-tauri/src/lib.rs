...
struct BootPlan {
    ...
    /// ~/.OpenTron/config.toml so `tron serve` / `Tron serve` picks it up.
    engine_host: Option<(String, String)>,
    /// Args appended after `uv run tron serve --port <port>`.
...
fn boot_plan(cfg: &InferenceConfig, ram_gb: f64) -> BootPlan {
    match cfg.kind {
        ...
        SourceKind::Custom => {
            ...
            // `model` may be empty if the config is malformed; `Tron serve`
            ...
            if let Some((_, host)) = &engine_host {
                serve_args.push("--engine-host".into());
                serve_args.push(host.clone());
            }
            BootPlan {
                launch_ollama: false,
                model_to_pull: None,
...
fn find_java_backend_root() -> Option<std::path::PathBuf> {
    if let Ok(root) = std::env::var("TRON_JAVA_ROOT").or_else(|_| std::env::var("OPENTRON_JAVA_ROOT")) {
    ...
    if let Some(root) = find_project_root() {
        for candidate_name in ["tron-java", "opentron-java"] {
            let candidate = root.join("java").join(candidate_name);
            if candidate.join("pom.xml").exists() {
...
fn find_java_backend() -> Option<(String, std::path::PathBuf)> {
    let java_bin = resolve_bin("java");
    if !std::path::Path::new(&java_bin).exists() {
        return None;
    }

    if let Ok(jar_path) = std::env::var("TRON_JAVA_JAR").or_else(|_| std::env::var("OPENTRON_JAVA_JAR")) {
    ...
    let java_root = find_java_backend_root()?;
    
    // Try CLI module JAR first (new multi-module structure)
    for jar_name in ["tron-cli-jar-with-dependencies.jar", "opentron-java-cli-0.1.0.jar"] {
        let jar = java_root.join("cli").join("target").join(jar_name);
        if jar.exists() {
            return Some((java_bin, jar));
        }
    }
    
    // Fall back to old structure (single module)
    for jar_name in ["tron-java-0.1.0.jar", "opentron-java-0.1.0.jar"] {
        let jar = java_root.join("target").join(jar_name);
        if jar.exists() {
            return Some((java_bin, jar));
        }
    }
    ...
    let cli_classes = java_root.join("cli").join("target").join("classes");
    if cli_classes.exists() {
        return Some((java_bin, cli_classes));
    }

    let classes = java_root.join("target").join("classes");
    if classes.exists() {
        return Some((java_bin, classes));
    }
...
fn build_java_tron_command(
    ...
    // Skip the Python wrapper commands ("run" and "tron") when passing to Java
    let java_args = if args.len() >= 3 && args[0] == "run" && args[1] == "tron" {
        &args[2..]
    } else {
        args
    };
...
/// Find the Tron project root (contains pyproject.toml).
/// Checks TRON_ROOT / OPENTRON_ROOT env vars, walks up from the executable,
/// then probes common clone locations.
fn find_project_root() -> Option<std::path::PathBuf> {
    // 1. Explicit env var override
    if let Ok(root) = std::env::var("TRON_ROOT").or_else(|_| std::env::var("OPENTRON_ROOT")) {
        let path = std::path::PathBuf::from(&root);
    ...
    let direct = [
        format!("{home}/Tron"),
        format!("{home}/tron"),
        format!("{home}/opentron"),
        format!("{home}/OpenTron"),
        format!("{home}/projects/hazy/OpenTron"),
        format!("{home}/projects/OpenTron"),
        format!("{home}/src/OpenTron"),
        format!("{home}/Documents/OpenTron"),
        format!("{home}/Desktop/OpenTron"),
        format!("{home}/Developer/OpenTron"),
        format!("{home}/dev/OpenTron"),
        format!("{home}/Code/OpenTron"),
        format!("{home}/code/OpenTron"),
        format!("{home}/repos/OpenTron"),
        format!("{home}/github/OpenTron"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for Tron or OpenTron one level inside common
    //    parent dirs. This catches clones like ~/Documents/my-stuff/OpenTron
    //    without needing to enumerate every possible intermediate folder.
    let scan_parents = [
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                for candidate_name in ["Tron", "tron", "OpenTron"] {
                    let candidate = entry.path().join(candidate_name);
                    if candidate.join("pyproject.toml").exists() {
                        return Some(candidate);
                    }
                }
                // Also check if the entry itself is OpenTron/Tron (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("OpenTron")
                        || name.eq_ignore_ascii_case("tron")
                        || name.eq_ignore_ascii_case("tron-desktop")
                    {
...
// ---------------------------------------------------------------------------
// BackendManager � owns the Ollama + Tron server child processes
// ---------------------------------------------------------------------------
...
/// Rolling buffer holding the most recent ~16 KB of tron / Tron stderr.
///
/// Populated by a background drainer task spawned at boot so the pipe
/// never fills and back-pressures `tron serve` / `Tron serve`; consumed by
/// the boot path when surfacing failure messages.
...
struct BackendManager {
    ...
    Tron: Option<ChildHandle>,
    Tron_stderr_tail: StderrTail,
}
...
impl Default for BackendManager {
    fn default() -> Self {
        Self {
            ollama: None,
            Tron: None,
            Tron_stderr_tail: Arc::new(Mutex::new(Vec::new())),
        }
    }
}
...
impl BackendManager {
    async fn stop_all(&mut self) {
        if let Some(ref mut h) = self.Tron {
            h.kill().await;
        }
        self.Tron = None;
...
/// Outcome of waiting for `tron serve` / `Tron serve` to become healthy.
///
...
fn spawn_Tron_stderr_drainer(mut stderr: tokio::process::ChildStderr, tail: StderrTail) {
...
async fn read_Tron_stderr_tail(backend: &SharedBackend) -> String {
    let tail = backend.lock().await.Tron_stderr_tail.clone();
    let bytes = tail.lock().await.clone();
    String::from_utf8_lossy(&bytes).trim().to_string()
}

/// Poll `Tron serve` health, watching the child process state so we
...
async fn wait_for_Tron_health(
    ...
    {
        Ok(c) => c,
        Err(_) => return TronStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    let mut probe_urls = vec![url.to_string()];
    if let Some(base) = url.strip_suffix("/actuator/health") {
        probe_urls.push(format!("{}/health", base));
        probe_urls.push(format!("{}/v1/savings", base));
    }
    loop {
        ...
        let exit_status = {
            ...
            match mgr.Tron.as_mut() {
                Some(h) => h.child.try_wait().ok().flatten(),
                None => None,
            }
        };
        if let Some(status) = exit_status {
            let stderr = read_Tron_stderr_tail(backend).await;
            return TronStartResult::EarlyExit {
                code: status.code(),
                stderr,
            };
        }
        ...
        for probe_url in &probe_urls {
            match client.get(probe_url).send().await {
                Ok(resp) => {
                    had_connectivity = true;
                    let status = resp.status();
                    if status.is_success() {
                        return TronStartResult::Ready;
                    }
                    if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                        ...
                        return TronStartResult::ServiceUnavailable(body);
                    }
                    // Non-2xx from one probe may still be fine for older
                    // backends; keep trying the remaining probe URLs.
                }
                Err(_) => {
                    // Ignore and keep trying other probe URLs.
                }
            }
        }
        if !had_connectivity {
            // Connection refused / DNS / timeout � server still booting.
            // Keep polling.
        }

        if tokio::time::Instant::now() >= deadline {
            return TronStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
///
/// When the OpenTron desktop binary is shipped as an AppImage, the AppImage
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if plan.launch_ollama {
        ...
        // Point `tron serve` / `Tron serve` at the user's endpoint by
        // writing the engine host into ~/.OpenTron/config.toml. The current
        // Python backend still reads the legacy OpenTron config path there.
    ...
    // Phase 3: Start tron serve / Tron serve
    {
        let mut s = status.lock().await;
        s.phase = "server".into();
...
let target_path = std::path::PathBuf::from(home_dir()).join("Tron");
    ...
    if target_path.exists() && !target_path.join("pyproject.toml").exists() {
        let mut s = status.lock().await;
        s.error = Some(format!(
            "{} exists but is not a valid Tron project. \
                 Remove it and relaunch, or set TRON_ROOT / OpenTron_ROOT to the correct path.",
            clone_target,
        ));
        return;
    }
    ...
    {
        let mut s = status.lock().await;
        s.detail = "Downloading Tron (first launch)...".into();
        }
        ...
        let clone_result = tokio::process::Command::new(&git_bin)
            .args([
                ...
                "--depth",
                "1",
                "https://github.com/open-Tron/OpenTron.git",
                &clone_target,
            ])
            .stdout(std::process::Stdio::null())
        ...
        match clone_result {
            Ok(child) => match child.wait_with_output().await {
                ...
                Ok(output) => {
                    ...
                    let mut s = status.lock().await;
                    s.error = Some(format!(
                        "Failed to download Tron from the OpenTron mirror: {}. \
                         Clone manually: git clone https://github.com/open-Tron/OpenTron.git {}",
                        stderr.trim(),
                        clone_target,
                    ));
                    return;
    ...
    // already started `tron serve` / `Tron serve` in a terminal and then
    // launched the desktop app � the app killed their server, then raced to
    // spawn its own, sometimes losing the race and hanging.
    //
    // New behaviour, by response shape:
    //   * 2xx /health        � healthy tron serve / Tron serve. Attach to it;
    //                          skip the uv-sync + spawn dance entirely. Done.
    //   * 503                � server is up but engine isn't ready. Surface
    //                          an actionable message; don't kill (matches
    //                          our wait_for_Tron_health 503 contract).
    ...
    // override from config instead of hard-coding TRON_PORT.
    {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        let probe_urls = vec![
            format!("http://127.0.0.1:{}/actuator/health", TRON_PORT),
            format!("http://127.0.0.1:{}/health", TRON_PORT),
            format!("http://127.0.0.1:{}/v1/savings", TRON_PORT),
        ];
        let mut attachable = false;
        let mut saw_503 = false;
        let mut last_non_ok_status: Option<reqwest::StatusCode> = None;
        for url in &probe_urls {
            match client.get(url).send().await {
                ...
                Ok(resp) if resp.status() == reqwest::StatusCode::SERVICE_UNAVAILABLE => {
                    saw_503 = true;
                    last_non_ok_status = Some(resp.status());
                }
                Ok(resp) => {
                    last_non_ok_status = Some(resp.status());
                }
        ...
        match (attachable, saw_503, last_non_ok_status) {
            (true, _, _) => {
            ...
            (false, true, _) => {
                ...
                s.error = Some(format!(
                    "An API server is already running on port {} but its \
                     ...
                     `tron serve`, wait for it to finish loading and relaunch. \
                     Otherwise, stop that service or change the port.",
                    TRON_PORT,
                ));
                return;
            }
            (false, false, Some(status_code)) => {
...

    {
        ...
        s.detail = format!(
            "Starting API server{}...",
            root.map(|r| format!(" from {}", r.display())).unwrap_or_default()
        );
    }
    ...
    // Extract engine-host if present in plan.serve_args
    // Java Serve class only recognizes: --port, --host, --engine-host
    if let Some((_, host)) = &plan.engine_host {
        serve_argv.push("--engine-host".into());
    ...
    // Inject cloud API keys from ~/.OpenTron/cloud-keys.env
    for (key, value) in read_cloud_keys() {
        cmd.env(&key, &value);
    }
    
    // Inject speech backend configuration from ~/.OpenTron/speech-config.json
    let speech_cfg = read_speech_config();
    if let Some(backend) = speech_cfg.backend {
        if !backend.is_empty() {
            cmd.env("SPEECH_BACKEND", backend);
        }
    }
    if let Some(openai_key) = speech_cfg.openai_key {
        if !openai_key.is_empty() {
            cmd.env("SPEECH_OPENAI_KEY", openai_key);
        }
    }
    ...
    match Tron_child {
        Ok(mut child) => {
            ...
            let mut mgr = backend.lock().await;
            let tail = mgr.Tron_stderr_tail.clone();
            mgr.Tron = Some(ChildHandle { child });
            drop(mgr);
        ...
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start Tron server: {}. \n\n\
                 Make sure the Java backend is built and configured, and that TRON_JAVA_ROOT / OPENTRON_JAVA_ROOT or TRON_JAVA_JAR points to the CLI jar or compiled classes.",
                e,
            ));
            return;
        }
    ...
    let server_url = format!("http://127.0.0.1:{}/actuator/health", TRON_PORT);
    match wait_for_Tron_health(&server_url, Duration::from_secs(600), &backend).await {
        TronStartResult::Ready => {}
        TronStartResult::ServiceUnavailable(body) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Tron server is running but the inference engine is not available \
                 (HTTP 503). This usually means the configured model couldn't be loaded.\n\n\
                 Check the server logs, or run 'uv run tron serve --port {}{}' \
                ...
                root.as_ref().map(|r| r.display().to_string()).unwrap_or_else(|| "<project-root>".to_string()),
                body.trim(),
            ));
            return;
        }
        TronStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Tron server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that the Java backend is built, the CLI jar or classes are available, and TRON_JAVA_ROOT / OPENTRON_JAVA_ROOT or TRON_JAVA_JAR is configured correctly.",
                    code_str,
                )
            } else {
                format!(
        ...
        TronStartResult::Timeout => {
            let stderr = read_Tron_stderr_tail(&backend).await;
            let mut s = status.lock().await;
            s.error = Some(if stderr.is_empty() {
                "Tron server did not become ready within 10 minutes. Check that the Java backend is built, the CLI jar or classes are available, and TRON_JAVA_ROOT / OPENTRON_JAVA_ROOT or TRON_JAVA_JAR is configured correctly.".into()
            } else {
                format!(
                    "Tron server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
async fn check_health(api_url: String) -> Result<serde_json::Value, String> {
    ...
    for url in urls {
        match reqwest::get(&url).await {
            Ok(resp) => {
                if !resp.status().is_success() {
                    last_err = Some(format!("Health probe {} returned {}", url, resp.status()));
                    continue;
                }
                return match resp.json::<serde_json::Value>().await {
                    Ok(v) => Ok(v),
                    Err(_) => Ok(serde_json::json!({ "status": "UP" })),
                };
            }
            Err(e) => {
                last_err = Some(format!("Connection failed for {}: {}", url, e));
            }
        }
    }

    Err(last_err.unwrap_or_else(|| "Health check failed".into()))
}
...
#[tauri::command]
async fn run_Tron_command(args: Vec<String>) -> Result<String, String> {
    let project_root = find_project_root();
    let mut cmd = if let Some(java_cmd) = build_java_tron_command(&args, project_root.as_ref()) {
        java_cmd
    } else {
        return Err(
            "Could not find the Java OpenTron backend. Build the Java CLI/backend and set TRON_JAVA_ROOT / OPENTRON_JAVA_ROOT or TRON_JAVA_JAR.".into(),
        );
    };

    // Run from the project root so `uv run tron` or the Java wrapper resolves
    // the Tron/OpenTron project regardless of the app's launch cwd. In a packaged
    // install the cwd isn't the checkout, so without this the CLI isn't found
    // and the UI then shows "Failed to get response" (see #531).
    ...
    if !is_serve {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("Failed to launch tron: {}", e))?;
    ...
    // `tron serve` / `Tron serve` is a long-running server that never exits.
    // The old code used `.output()`, which waits for the process to exit and so
    // hung this command forever � the "Start" button never resolved (#531).
    // Spawn it detached instead, drain stderr (a full 4 KB Windows pipe can
    // otherwise stall the child mid-startup, #309), and poll /health for readiness.
    cmd.stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::piped());
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("Failed to launch tron serve: {}", e))?;

    let tail: StderrTail = Arc::new(Mutex::new(Vec::new()));
    if let Some(stderr) = child.stderr.take() {
        spawn_Tron_stderr_drainer(stderr, tail.clone());
    }
    ...
    let url = format!("http://127.0.0.1:{}/health", TRON_PORT);
    ...
    loop {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(format!(
                "tron serve exited (code {:?}) before becoming healthy:\n{}",
                status.code(),
                stderr.trim()
            ));
        }
        if let Ok(resp) = client.get(&url).send().await {
            if resp.status().is_success() {
                ...
                return Ok(format!(
                    "tron serve is ready on http://127.0.0.1:{}",
                    TRON_PORT
                ));
            }
        }
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "tron serve did not become healthy on port {} within 120s.",
                TRON_PORT
            ));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
...
/// Path to the cloud keys file (~/.OpenTron/cloud-keys.env).
...
async fn save_cloud_key(key_name: String, key_value: String) -> Result<(), String> {
    ...
    let reload_url = format!("http://127.0.0.1:{}/v1/cloud/reload", TRON_PORT);
...
// ---------------------------------------------------------------------------
// Inference-source selection (~/.OpenTron/inference.json)
// ---------------------------------------------------------------------------
...
/// Path to the inference-source config (~/.OpenTron/inference.json).
...
/// Write the custom-endpoint host into ~/.OpenTron/config.toml so
/// `tron serve` / `Tron serve` (which reads that file via load_config)
/// points at it.
...
// ---------------------------------------------------------------------------
// Speech configuration (~/.OpenTron/speech-config.json)
// ---------------------------------------------------------------------------

#[derive(serde::Serialize, serde::Deserialize, Clone, Debug, Default)]
struct SpeechConfig {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    backend: Option<String>, // "none", "openai", or "ollama"
    #[serde(default, skip_serializing_if = "Option::is_none")]
    openai_key: Option<String>,
}

/// Path to the speech config file (~/.OpenTron/speech-config.json).
fn speech_config_path() -> std::path::PathBuf {
    std::path::PathBuf::from(home_dir())
        .join(".OpenTron")
        .join("speech-config.json")
}

/// Read speech config from disk, or default if absent.
fn read_speech_config() -> SpeechConfig {
    let path = speech_config_path();
    match std::fs::read_to_string(&path) {
        Ok(text) => serde_json::from_str::<SpeechConfig>(&text).unwrap_or_default(),
        Err(_) => SpeechConfig::default(),
    }
}

/// Save speech config to disk.
fn write_speech_config(cfg: &SpeechConfig) -> Result<(), String> {
    let path = speech_config_path();
    if let Some(parent) = path.parent() {
        let _ = std::fs::create_dir_all(parent);
    }
    let json = serde_json::to_string_pretty(cfg).map_err(|e| e.to_string())?;
    std::fs::write(&path, json + "\n").map_err(|e| format!("Failed to save speech config: {}", e))
}

/// Save speech settings from the frontend (called when user saves in Settings).
#[tauri::command]
async fn save_speech_config_cmd(
    backend: String,
    openai_key: String,
) -> Result<(), String> {
    let cfg = SpeechConfig {
        backend: if backend.is_empty() || backend == "none" {
            None
        } else {
            Some(backend)
        },
        openai_key: if openai_key.is_empty() { None } else { Some(openai_key) },
    };
    write_speech_config(&cfg)
}
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("TronOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("TronOverlayPanel", sup).unwrap();
        ...
        if Class::get("TronOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("TronOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("TronOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("TronOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("TronOverlayPanel").unwrap();
        ...
        let hcls = Class::get("TronOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("TronOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit Tron").build(app)?;
            ...
            #[cfg(target_os = "macos")]
            unsafe {
                native_overlay::create(include_str!("overlay.html"), TRON_PORT);
            }
        ...
        .expect("error while building Tron Desktop")
        .run(move |_app, event| {
...
mod tests {
    ...
    fn tail_never_splits_a_multibyte_codepoint() {
        // Each "�" is 2 bytes / 1 char. A byte-based slice could panic or
    ...
    fn failure_message_includes_exit_code_and_tail_and_hint() {
        let msg = format_uv_sync_failure(
            Path::new("/home/u/.OpenTron/src"),
            Some(2),
            "error: failed to resolve numpy==2.1.3",
        );
        assert!(msg.contains("exit 2"));
        assert!(msg.contains("/home/u/.OpenTron/src"));
    ...
    fn boot_plan_custom_skips_ollama_and_sets_engine_host() {
        ...
        assert!(plan.serve_args.windows(2).any(|w| w == ["--engine-host", "http://localhost:1234"]));
    }
