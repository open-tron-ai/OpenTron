...
are local fakes, so you can ``python examples/OpenTron/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/OpenTron/examples/spec-search-quickstart.toml``
via the regular ``OpenTron.core.config.load_config`` machinery — you can copy
that TOML to ``~/.OpenTron/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    OpenTron_HOME=/tmp/OpenTron-spec-search-demo \\
...
from OpenTron.core.config import SpecSearchLearningConfig
from OpenTron.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from OpenTron.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from OpenTron.learning.spec_search.multi_session import SpecSearchLoop
from OpenTron.learning.spec_search.orchestrator import SpecSearchOrchestrator
from OpenTron.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from OpenTron.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = OpenTron.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.OpenTron/config.toml``.
    ...
    #
    # (after copying ``configs/OpenTron/examples/spec-search-quickstart.toml``
    # to ``~/.OpenTron/config.toml``)
    ...
    home = Path(
        os.environ.get("OpenTron_HOME")
        or tempfile.mkdtemp(prefix="OpenTron-spec-search-")
    )
    print(f"OpenTron_HOME = {home}")
    ...
    with patch(
        "OpenTron.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
