...
This tutorial demonstrates how to use OpenTron to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Tron` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `Tron scheduler` CLI
...
Register each script as a recurring task with `Tron scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | Tron channel send slack
...
See `Tron channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let OpenTron auto-select from what is available.
