...
A set of developer-focused scripts that use OpenTron tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Tron` instance with the `native_react` (ReAct) agent
...
1. **Install OpenTron** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/OpenTron/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/OpenTron/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run Tron eval list` or
inspect `src/OpenTron/tools/` for the full registry.
