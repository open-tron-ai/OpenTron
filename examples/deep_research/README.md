...
using OpenTron. The assistant uses an orchestrator agent loop with web
...
- OpenTron installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `Tron eval`
runner.
...
User query
  |
  v
Tron SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `Tron agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `OpenTron.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/OpenTron/recipes/) -- composable TOML configs that
