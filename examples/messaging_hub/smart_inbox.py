...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: Tron channel list   to see available channels.\n",
    )
    ...
    click.echo("  from OpenTron import Tron")
    click.echo(f'  j = Tron(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: Tron channel status  (to verify "{channel}" is connected)')
