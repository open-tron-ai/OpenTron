...
OpenTron orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting OpenTron to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- OpenTron installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the OpenTron channel docs).
...
OpenTron supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/OpenTron/config.toml` for the full list of channel and agent
options.
...
this via the OpenTron scheduler:

```bash
Tron scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/OpenTron/recipes/data/operators/` for examples.
