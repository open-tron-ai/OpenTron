...
All notable changes to OpenTron are documented in this file.
...
**Vision input for `Tron ask`** — attach images to a query with
...
`Tron_NUM_CTX` environment variable to tune the Ollama context window
(default `16384`).
...
install path while `OpenTron.ai` is down, improves desktop
first-boot diagnostics on Windows, and ships the RAM-detection fix
for Windows that missed the v1.0.1 cutoff.

### Fixed

**`OpenTron/traces/` missing from the v1.0.1 PyPI wheel** (#372).
...
`src/OpenTron/traces/` — silently dropping the whole package. Every
fresh `pip install OpenTron==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'OpenTron.traces'` on the
first `Tron ask`, learning, or server call. Anchored the pattern to
...
the v1.0.1 cutoff, so v1.0.1 users still saw `0.0 GB` from `Tron
...
`OpenTron.ai/install.sh` URL was failing with `sslv3 alert
...
`https://open-Tron.github.io/OpenTron/install.sh`, generated from
...
**`Tron self-update`** subcommand. Detects how OpenTron was
installed (pip, uv tool, editable git checkout) by inspecting
`OpenTron.__file__`, then runs the right upgrade command. Supports
...
(`OpenTron_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `OpenTron_NO_ANALYTICS`).
...
- **Editable-git users running `Tron self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/OpenTron`. If you'd come to rely on the
...
Companion blog post:
[From Minions to OpenTron: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-OpenTron-retrospective).
...
`TronSystem` in `src/OpenTron/system.py`, driven by a TOML config.
...
`Tron init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/OpenTron/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/OpenTron/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/OpenTron/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/OpenTron/agents/hybrid/`.
...
A runner CLI (`python -m OpenTron.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.OpenTron/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.OpenTron/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `TronAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `Tron skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `Tron skill discover` — mine traces for recurring tool patterns
  - `Tron skill show-overlay` — inspect optimization output
  - `Tron optimize skills` — run DSPy/GEPA per-skill optimization
  - `Tron bench skills` — run the PinchBench skills benchmark
...
- `examples/OpenTron/spec_search_quickstart.py` — runnable end-to-end
...
- `src/OpenTron/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from OpenTron.learning.distillation.*` →
  `from OpenTron.learning.spec_search.*`). The `Tron distillation`
...
- **Engine `generate_full` return shape extended.**
  `TronAgentBackend.generate_full` and `TronDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `Tron skill install openclaw:owner/slug` now correctly splits into category + name match.
