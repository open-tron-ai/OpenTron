...
use OpenTron_core::config::TronConfig;
...
pub fn discover_engines(config: &TronConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, OpenTronError> {
    ...
    match key.as_str() {
        ...
        other => Err(OpenTronError::Engine(
            OpenTron_core::error::EngineError::ModelNotFound(format!(
