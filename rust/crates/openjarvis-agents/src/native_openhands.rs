...
use OpenTron_core::{AgentContext, AgentResult, OpenTronError, ToolResult};
use OpenTron_tools::executor::ToolExecutor;
use regex::Regex;
...
impl<M: CompletionModel + 'static> OjAgent for NativeOpenHandsAgent<M> {
    ...
    async fn run(
    ...
    ) -> Result<AgentResult, OpenTronError> {
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        OpenTron_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        OpenTron_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    OpenTronError::Agent(OpenTron_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<OpenTron_engine::Engine>>;
