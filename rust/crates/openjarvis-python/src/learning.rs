...
use OpenTron_learning::RouterPolicy;
use pyo3::prelude::*;

#[pyclass(name = "HeuristicRouter")]
pub struct PyHeuristicRouter {
    inner: OpenTron_learning::HeuristicRouter,
}
...
impl PyHeuristicRouter {
    ...
    fn new(
        ...
        Self {
            inner: OpenTron_learning::HeuristicRouter::new(
    ...
    fn select_model(&self, query: &str, has_code: bool, has_math: bool) -> String {
        let ctx = OpenTron_core::RoutingContext {
...
pub struct PyBanditRouterPolicy {
    inner: OpenTron_learning::BanditRouterPolicy,
}
...
impl PyBanditRouterPolicy {
    ...
    fn new(models: Vec<String>, strategy: &str) -> Self {
        let strat = match strategy {
            "ucb1" | "UCB1" => OpenTron_learning::bandit::BanditStrategy::UCB1,
            _ => OpenTron_learning::bandit::BanditStrategy::ThompsonSampling,
        };
        Self {
            inner: OpenTron_learning::BanditRouterPolicy::new(models, strat),
        }
    }

    fn select_model(&self) -> String {
        let ctx = OpenTron_core::RoutingContext::default();
        self.inner.select_model(&ctx)
    }
...
pub struct PyGRPORouterPolicy {
    inner: OpenTron_learning::GRPORouterPolicy,
}
...
impl PyGRPORouterPolicy {
    ...
    fn new(models: Vec<String>, temperature: f64) -> Self {
        Self {
            inner: OpenTron_learning::GRPORouterPolicy::new(models, temperature),
        }
    }
...
pub struct PyOptimizationStore {
    inner: parking_lot::Mutex<OpenTron_learning::optimize::OptimizationStore>,
}
...
impl PyOptimizationStore {
    ...
    fn new(path: &str) -> PyResult<Self> {
        let store = if path == ":memory:" {
            OpenTron_learning::optimize::OptimizationStore::in_memory()
        } else {
            OpenTron_learning::optimize::OptimizationStore::open(path)
        };
        let store =
    ...
    fn save_trial(&self, run_id: &str, trial_json: &str) -> PyResult<()> {
        let trial: OpenTron_learning::optimize::TrialResult =
...
pub struct PyLLMOptimizer {
    inner: OpenTron_learning::optimize::LLMOptimizer,
}
...
impl PyLLMOptimizer {
    ...
    fn new(search_space_json: &str, optimizer_model: &str) -> PyResult<Self> {
        let search_space: OpenTron_learning::optimize::SearchSpace =
            serde_json::from_str(search_space_json)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
        let inner = OpenTron_learning::optimize::LLMOptimizer::new(
            search_space,
    ...
    fn propose_next(&self, history_json: &str) -> PyResult<String> {
        let history: Vec<OpenTron_learning::optimize::TrialResult> =
...
pub struct PySFTRouterPolicy {
    inner: OpenTron_learning::SFTRouterPolicy,
}
...
impl PySFTRouterPolicy {
    ...
    fn new(min_samples: usize) -> Self {
        Self {
            inner: OpenTron_learning::SFTRouterPolicy::new(min_samples),
        }
    }
    ...
    fn classify_query(query: &str) -> &'static str {
        OpenTron_learning::SFTRouterPolicy::classify_query(query)
    }
...
pub struct PyHeuristicRewardFunction {
    inner: OpenTron_learning::HeuristicRewardFunction,
}
...
impl PyHeuristicRewardFunction {
    ...
    fn new(
        ...
        Self {
            inner: OpenTron_learning::HeuristicRewardFunction::new(
                weight_latency,
                weight_cost,
                weight_efficiency,
                max_latency,
...
pub struct PySkillDiscovery {
    inner: OpenTron_learning::SkillDiscovery,
}
...
impl PySkillDiscovery {
    ...
    fn new(
        ...
        Self {
            inner: OpenTron_learning::SkillDiscovery::new(
                min_frequency,
                min_sequence_length,
                max_sequence_length,
                min_outcome,
...
pub struct PyTraceDrivenPolicy {
    inner: OpenTron_learning::TraceDrivenPolicy,
}
...
impl PyTraceDrivenPolicy {
    ...
    fn new(available_models: Vec<String>, default_model: &str, fallback_model: &str) -> Self {
        Self {
            inner: OpenTron_learning::TraceDrivenPolicy::new(
                available_models,
                default_model.to_string(),
...
pub struct PyAgentAdvisorPolicy {
    inner: OpenTron_learning::AgentAdvisorPolicy,
}
...
impl PyAgentAdvisorPolicy {
    ...
    fn new(max_traces: usize) -> Self {
        Self {
            inner: OpenTron_learning::AgentAdvisorPolicy::new(max_traces),
        }
    }

    fn analyze_patterns(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<OpenTron_learning::TraceInfo> = serde_json::from_str(traces_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
    ...
    fn classify(query: &str) -> &'static str {
        OpenTron_learning::AgentAdvisorPolicy::classify(query)
    }
}
...
pub struct PyICLUpdaterPolicy {
    inner: OpenTron_learning::ICLUpdaterPolicy,
}
...
impl PyICLUpdaterPolicy {
    ...
    fn new(min_score: f64, max_examples: usize, min_skill_occurrences: usize) -> Self {
        Self {
            inner: OpenTron_learning::ICLUpdaterPolicy::new(
                min_score,
                max_examples,
...
pub struct PyTrainingDataMiner {
    inner: OpenTron_learning::TrainingDataMiner,
}
...
impl PyTrainingDataMiner {
    ...
    fn new(min_quality: f64, min_samples_per_class: usize) -> Self {
        Self {
            inner: OpenTron_learning::TrainingDataMiner::new(min_quality, min_samples_per_class),
        }
    }

    fn extract_sft_pairs(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<OpenTron_learning::MinerTraceData> = serde_json::from_str(traces_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyAgentConfigEvolver {
    inner: OpenTron_learning::AgentConfigEvolver,
}
...
impl PyAgentConfigEvolver {
    ...
    fn new(min_quality: f64) -> Self {
        Self {
            inner: OpenTron_learning::AgentConfigEvolver::new(min_quality),
        }
    }

    fn analyze(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<OpenTron_learning::EvolutionTraceData> =
            serde_json::from_str(traces_json)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyMultiObjectiveReward {
    inner: OpenTron_learning::MultiObjectiveReward,
}
...
impl PyMultiObjectiveReward {
    ...
    fn new() -> Self {
        Self {
            inner: OpenTron_learning::MultiObjectiveReward::new(
                OpenTron_learning::RewardWeights::default(),
                OpenTron_learning::Normalizers::default(),
            ),
        }
    }

    fn compute(&self, episode_json: &str) -> PyResult<f64> {
        let ep: OpenTron_learning::Episode = serde_json::from_str(episode_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyLearningOrchestrator {
    inner: OpenTron_learning::LearningOrchestrator,
}
...
impl PyLearningOrchestrator {
    ...
    fn new(min_improvement: f64, min_sft_pairs: usize, min_quality: f64) -> Self {
        Self {
            inner: OpenTron_learning::LearningOrchestrator::new(
                min_improvement,
                min_sft_pairs,
                min_quality,
            ),
