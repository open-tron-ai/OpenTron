//! PyO3 bridge — exposes ~50 Rust classes to Python via `OpenTron_rust`.
...
fn load_config(path: Option<&str>) -> PyResult<core::PyConfig> {
    ...
    let config = OpenTron_core::load_config(p)
...
fn detect_hardware() -> PyResult<String> {
    let hw = OpenTron_core::hardware::detect_hardware();
    Ok(serde_json::to_string(&hw).unwrap_or_default())
}

#[pyfunction]
fn check_ssrf(url: &str) -> Option<String> {
    OpenTron_security::check_ssrf(url)
}

#[pyfunction]
fn is_sensitive_file(path: &str) -> bool {
    OpenTron_security::is_sensitive_file(std::path::Path::new(path))
}

#[pyfunction]
fn register_builtin_models() {
    OpenTron_core::model_catalog::register_builtin_models();
}

#[pyfunction]
fn classify_query(query: &str) -> &'static str {
    OpenTron_learning::classify_query(query)
}

#[pymodule]
fn OpenTron_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // --- Core types ---
