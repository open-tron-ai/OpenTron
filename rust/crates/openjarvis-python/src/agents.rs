...
use crate::RUNTIME;
use OpenTron_agents::OjAgent;
use OpenTron_engine::rig_adapter::RigModelAdapter;
use OpenTron_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(OpenTron_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(OpenTron_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(OpenTron_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&OpenTron_core::AgentContext>,
    ) -> Result<OpenTron_core::AgentResult, OpenTron_core::OpenTronError> {
        match self {
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = OpenTron_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(OpenTron_tools::ToolExecutor::new(None, None));
        let agent = OpenTron_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(OpenTron_tools::ToolExecutor::new(None, None));
        let agent = OpenTron_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = OpenTron_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = OpenTron_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(OpenTron_tools::ToolExecutor::new(None, None));
        let agent = OpenTron_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => OpenTron_agents::MemoryExtraction::Scratchpad,
            "structured_json" => OpenTron_agents::MemoryExtraction::StructuredJson,
            "none" => OpenTron_agents::MemoryExtraction::None,
            _ => OpenTron_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => OpenTron_agents::ObservationCompression::Truncate,
            "none" => OpenTron_agents::ObservationCompression::None,
            _ => OpenTron_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => OpenTron_agents::RetrievalStrategy::Keyword,
            "semantic" => OpenTron_agents::RetrievalStrategy::Semantic,
            "none" => OpenTron_agents::RetrievalStrategy::None,
            _ => OpenTron_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => OpenTron_agents::TaskDecomposition::Monolithic,
            "hierarchical" => OpenTron_agents::TaskDecomposition::Hierarchical,
            _ => OpenTron_agents::TaskDecomposition::Phased,
        };

        let monitor_config = OpenTron_agents::MonitorConfig {
        ...
        let agent = OpenTron_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: OpenTron_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: OpenTron_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
