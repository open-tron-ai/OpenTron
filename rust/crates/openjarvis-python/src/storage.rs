...
use OpenTron_tools::storage::MemoryBackend;
use pyo3::prelude::*;

#[pyclass(name = "SQLiteMemory")]
pub struct PySQLiteMemory {
    inner: OpenTron_tools::storage::SQLiteMemory,
}
...
impl PySQLiteMemory {
    ...
    fn new(path: &str) -> PyResult<Self> {
        let inner = OpenTron_tools::storage::SQLiteMemory::new(std::path::Path::new(path))
...
pub struct PyBM25Memory {
    inner: OpenTron_tools::storage::BM25Memory,
}
...
impl PyBM25Memory {
    ...
    fn new(k1: f64, b: f64) -> Self {
        Self {
            inner: OpenTron_tools::storage::BM25Memory::new(k1, b),
        }
    }
...
pub struct PyFAISSMemory {
    inner: OpenTron_tools::storage::FAISSMemory,
}
...
impl PyFAISSMemory {
    ...
    fn new(path: &str, dim: usize) -> PyResult<Self> {
        let inner = OpenTron_tools::storage::FAISSMemory::new(std::path::Path::new(path), dim)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }
...
pub struct PyColBERTMemory {
    inner: OpenTron_tools::storage::ColBERTMemory,
}
...
impl PyColBERTMemory {
    ...
    fn new(path: &str, token_dim: usize) -> PyResult<Self> {
        let inner =
            OpenTron_tools::storage::ColBERTMemory::new(std::path::Path::new(path), token_dim)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }
...
pub struct PyHybridMemory {
    inner: OpenTron_tools::storage::HybridMemory,
}
    ...
    fn new(backend_keys: Vec<String>) -> PyResult<Self> {
        ...
        for key in &backend_keys {
            let backend: Box<dyn MemoryBackend> = match key.as_str() {
                "sqlite" => {
                    let m = OpenTron_tools::storage::SQLiteMemory::new(
                ...
                "bm25" => Box::new(OpenTron_tools::storage::BM25Memory::default()),
                "faiss" => {
                    let m = OpenTron_tools::storage::FAISSMemory::in_memory().map_err(|e| {
                        PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
                    })?;
                    Box::new(m)
                }
                "colbert" => {
                    let m =
                        OpenTron_tools::storage::ColBERTMemory::in_memory().map_err(|e| {
                            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
                        })?;
                    Box::new(m)
                }
        ...
        Ok(Self {
            inner: OpenTron_tools::storage::HybridMemory::new(backends),
        })
    }
...
pub struct PyKnowledgeGraphMemory {
    inner: OpenTron_tools::storage::KnowledgeGraphMemory,
}
...
impl PyKnowledgeGraphMemory {
    ...
    #[pyo3(signature = (path=":memory:"))]
    fn new(path: &str) -> PyResult<Self> {
        let inner = OpenTron_tools::storage::KnowledgeGraphMemory::new(std::path::Path::new(path))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }
