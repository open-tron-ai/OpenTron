...
pub struct PyWorkflowGraph {
    inner: OpenTron_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: OpenTron_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => OpenTron_workflow::NodeType::Tool,
            "condition" => OpenTron_workflow::NodeType::Condition,
            "parallel" => OpenTron_workflow::NodeType::Parallel,
            "loop" => OpenTron_workflow::NodeType::Loop,
            "transform" => OpenTron_workflow::NodeType::Transform,
            _ => OpenTron_workflow::NodeType::Agent,
        };
        let node = OpenTron_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = OpenTron_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<OpenTron_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(OpenTron_workflow::WorkflowBuilder::new(name)),
        }
    }
