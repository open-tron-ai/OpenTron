...
impl PyAgentCard {
    ...
    fn new(name: &str, description: &str, version: &str, url: &str) -> Self {
        Self {
            inner: OpenTron_a2a::AgentCard::new(name, description, version, url),
        }
    }
...
pub struct PyA2ATaskStore {
    inner: OpenTron_a2a::A2ATaskStore,
}
...
impl PyA2ATaskStore {
    ...
    fn new() -> Self {
        Self {
            inner: OpenTron_a2a::A2ATaskStore::new(),
        }
    }
    ...
    fn update_state(&mut self, id: &str, state: &str) -> bool {
        let s = match state {
            "pending" => OpenTron_a2a::TaskState::Pending,
            "active" => OpenTron_a2a::TaskState::Active,
            "completed" => OpenTron_a2a::TaskState::Completed,
            "cancelled" => OpenTron_a2a::TaskState::Cancelled,
            "failed" => OpenTron_a2a::TaskState::Failed,
            _ => return false,
        };
...
pub fn parse_a2a_request(json_str: &str) -> PyResult<String> {
    let req = OpenTron_a2a::parse_request(json_str)
