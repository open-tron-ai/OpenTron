//! LLM-based optimizer for OpenTron configuration tuning.
//!
//! Rust translation of `src/OpenTron/learning/optimize/llm_optimizer.py`.
//!
...
/// Uses an LLM to propose optimal OpenTron configurations.
///
...
impl LLMOptimizer {
    ...
    pub fn analyze_trial(
        ...
        if let Some(ref backend) = self.backend {
            let prompt = format!(
                "Analyze this OpenTron evaluation result.\n\n\
                 ## Configuration\n{}\n\n\
                 ## Results\n- accuracy: {accuracy:.4}\n\
    ...
    fn build_initial_prompt(&self) -> String {
        ...
        lines.push("You are optimizing an OpenTron AI system configuration.".into());
