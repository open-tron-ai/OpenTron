...
//!
//! Rust translation of `src/OpenTron/core/config.py`.
...
/// Default config directory: `~/.OpenTron/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".OpenTron")
}

/// Default config file path: `~/.OpenTron/config.toml`
...
fn default_sandbox_image() -> String { "OpenTron-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.OpenTron/operators".into() }
...
// ---------------------------------------------------------------------------
// Top-level TronConfig
// ---------------------------------------------------------------------------

/// Top-level configuration for OpenTron.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
pub fn load_config(path: Option<&Path>) -> Result<TronConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: TronConfig = toml::from_str(&content)?;
...
mode = "block"
"#;
        let cfg: TronConfig = toml::from_str(toml_str).unwrap();
