...
use OpenTron_core::{OpenTronError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, OpenTronError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                OpenTronError::Io(std::io::Error::other(e))
            })?;
        }

        let conn = Connection::open(db_path).map_err(|e| {
            OpenTronError::Io(std::io::Error::other(e.to_string()))
        })?;
    ...
    pub fn in_memory() -> Result<Self, OpenTronError> {
        Self::new(Path::new(":memory:"))
    }

    pub fn save(&self, trace: &Trace) -> Result<(), OpenTronError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, OpenTronError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| {
                OpenTronError::Io(std::io::Error::other(
                    e.to_string(),
                ))
            })?;
    ...
    pub fn list_traces(
        ...
        limit: usize,
        offset: usize,
    ) -> Result<Vec<Trace>, OpenTronError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
                "SELECT trace_id, query, agent, model, engine, result, outcome, feedback,
    ...
    pub fn count(&self) -> Result<usize, OpenTronError> {
        let conn = self.conn.lock();
        let count: i64 = conn
