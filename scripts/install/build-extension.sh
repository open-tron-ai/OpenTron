...
#
# State files under $OpenTron_HOME/.state/:
...
MANIFEST="$SRC_DIR/rust/crates/OpenTron-python/Cargo.toml"
...
if uv run maturin develop -m "$MANIFEST" >>"$LOG" 2>&1; then
    ...
    if ! uv run python -c "import OpenTron_rust" >>"$LOG" 2>&1; then
        rc=$?
        {
            echo "build-extension.sh: maturin succeeded but 'import OpenTron_rust'"
