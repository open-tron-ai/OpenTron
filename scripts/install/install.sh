#!/usr/bin/env bash
# install.sh — OpenTron curl-pipe-bash installer.
#
# Usage:
...
#   OpenTron_HOME        Install dir (default: $HOME/.OpenTron)
#   OpenTron_REPO_URL    git repo URL (default: https://github.com/open-Tron/OpenTron.git)
#   OpenTron_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of OpenTron can't
...
OpenTron runs on Windows via WSL2. Two paths:
  ...
  2. Desktop app — download the .exe from the Releases page:
     https://github.com/open-Tron/OpenTron/releases

See the WSL2 install guide for the full walkthrough:
  https://open-Tron.github.io/OpenTron/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
OpenTron installs to $HOME/.OpenTron, not /usr/local. Re-run as your
...
Two ways forward:
  ...
  2. Pre-authenticate sudo before piping (caches credentials for 5 min):
       sudo -v && curl -fsSL https://open-Tron.github.io/OpenTron/install.sh | bash
EOF
        exit 1
    fi
    local sudo=""
...
# ---- env ----
# OpenTron keeps ALL of its state (install tree + runtime data, configs,
# databases, caches, logs) under a single root so it never clutters $HOME
# beyond one directory. Relocate it by exporting OpenTron_HOME before
# running the installer, e.g.:
#     OpenTron_HOME=~/apps/OpenTron curl ... | bash
# The Python runtime honors the same override (and, when OpenTron_HOME is
# unset, $XDG_DATA_HOME/OpenTron if XDG_DATA_HOME is set). With nothing set
# the root is ~/.OpenTron, so existing installs are untouched.
OpenTron_HOME="${OpenTron_HOME:-$HOME/.OpenTron}"
OpenTron_REPO_URL="${OpenTron_REPO_URL:-https://github.com/open-Tron/OpenTron.git}"
SRC_DIR="$OpenTron_HOME/src"
VENV_DIR="$OpenTron_HOME/.venv"
STATE_DIR="$OpenTron_HOME/.state"
SCRIPTS_DIR="$OpenTron_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$OpenTron_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${OpenTron_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# OpenTron team can see where users drop off during install.
...
#
ANALYTICS_HOST="${OpenTron_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${OpenTron_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$OpenTron_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$OpenTron_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/Tron" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/Tron-wrapper.sh" "$HOME/.local/bin/Tron"
    ln -sf "$SCRIPTS_DIR/Tron-uninstall.sh" "$HOME/.local/bin/Tron-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `Tron` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "OpenTron" "$rc" 2>/dev/null; then
...
from OpenTron.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "OpenTron installer"
echo "  install dir: $OpenTron_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone OpenTron repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install OpenTron"    editable_install
...
#
#   PATH ok + model ok   -> "type Tron"
#   PATH new + model ok  -> "source rc && Tron  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; Tron doctor"
#   PATH new + model bad -> "source rc && Tron doctor; chat works once download finishes"
#
# `Tron` and `Tron doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="Tron"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="Tron doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'Tron doctor'
...
Background work continues silently:
  ...
  Run 'Tron doctor' to check status anytime.
EOF
