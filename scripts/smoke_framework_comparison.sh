...
#   Tron_MOCK_LLM_URL - OpenAI-compatible endpoint (Ollama or vLLM)
#
# Optional:
#   Tron_ALLOW_COMMIT_DRIFT=1  - bypass commit-pin enforcement

set -euo pipefail

: "${HERMES_AGENT_PATH:?must be set}"
: "${OPENCLAW_PATH:?must be set}"
: "${Tron_MOCK_LLM_URL:?must be set (e.g. http://localhost:11434/v1)}"
...
uv run python -c "
from OpenTron.evals.comparison.third_party import (
...
SMOKE_FRAMEWORKS=(hermes openclaw OpenTron)
SMOKE_MODEL="qwen-9b"

for fwk in "${SMOKE_FRAMEWORKS[@]}"; do
  for bench in "${SMOKE_BENCHES[@]}"; do
    uv run python -m OpenTron.evals.comparison.make_configs \
      --framework "$fwk" \
      --model "$SMOKE_MODEL" \
      --benchmark "$bench" \
    ...
    Tron_BACKEND_BASE_URL="$Tron_MOCK_LLM_URL" uv run python - "$config" "$run_dir" <<'PY'
...
PY
    echo "  ▸ $fwk × $bench"
    mkdir -p "$run_dir"
    Tron_BACKEND_BASE_URL="$Tron_MOCK_LLM_URL" \
    Tron_BACKEND_API_KEY="${Tron_BACKEND_API_KEY:-smoke}" \
    uv run python -m OpenTron.evals run --config "$config" \
      || echo "    FAILED (continuing)"
  done
done

echo "==> Generating T1 from smoke results"
uv run python -m OpenTron.evals.comparison.table_gen \
