...
MODEL="${OpenTron_MODEL:-qwen3:0.6b}"
...
uv run maturin develop -m rust/crates/OpenTron-python/Cargo.toml --quiet 2>/dev/null \
...
uv run Tron serve --port 8000 &>/dev/null &
CLEANUP_PIDS+=($!)
sleep 3
...
echo ""
echo -e "${GREEN}${BOLD}  OpenTron is running!${NC}"
echo ""
