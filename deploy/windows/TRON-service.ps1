<#
.SYNOPSIS
    Register / unregister the OpenTron Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/OpenTron.service and
    deploy/launchd/com.OpenTron.plist.

    Registers a per-user scheduled task named "OpenTron" that starts
    `Tron serve` at logon and restarts on failure. Loopback default
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\OpenTron (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:OpenTron_API_KEY
                           — the server refuses to start unauthenticated
                           on a non-loopback bind.
      -ListenPort <int>    default: 8000

    Usage:
      powershell -ExecutionPolicy Bypass -File Tron-service.ps1 install
      powershell -ExecutionPolicy Bypass -File Tron-service.ps1 uninstall
      powershell -ExecutionPolicy Bypass -File Tron-service.ps1 status
#>
...
function Get-DefaultInstallRoot {
    ...
    if ($env:OpenTron_HOME) { return $env:OpenTron_HOME }
    return (Join-Path $env:LOCALAPPDATA 'OpenTron')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "OpenTron source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/OpenTron.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:OpenTron_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:OpenTron_API_KEY is
not set. An unauthenticated non-loopback bind is refused by Tron serve
and would also create a security hole. Set the env var first:

    `$env:OpenTron_API_KEY = (uv run Tron auth generate-key)
...
"@
    ...
    # `Tron serve` would refuse to bind 0.0.0.0 — failing silently every
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting OpenTron_API_KEY to User environment so the scheduled task can read it at logon."
        [System.Environment]::SetEnvironmentVariable(
            'OpenTron_API_KEY',
            $env:OpenTron_API_KEY,
            'User'
        )
    }
    ...
    $action = New-ScheduledTaskAction `
        -Execute $uvPath `
        -Argument "run Tron serve --host $ListenHost --port $ListenPort" `
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'OpenTron API server (loopback default — see deploy/windows/README.md)' | Out-Null
