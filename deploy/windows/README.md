...
5. Clones the OpenTron repository to `%LOCALAPPDATA%\OpenTron`
...
The available env vars: `OpenTron_SKIP_SERVICE`, `OpenTron_SERVICE`,
`OpenTron_FORCE`. If you need richer control, save the script first
...
remove the task with `Tron-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\OpenTron\src\deploy\windows\Tron-service.ps1"
...
`Tron-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:OpenTron_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/OpenTron/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/OpenTron.service` | `deploy/launchd/com.OpenTron.plist` | `deploy/windows/Tron-service.ps1` (cmdlet-driven) |
...
```powershell
cd "$env:LOCALAPPDATA\OpenTron\src"
git pull --ff-only
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\OpenTron\src\deploy\windows\Tron-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\OpenTron"
```
