<#
.SYNOPSIS
    OpenTron native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the OpenTron repository to $env:LOCALAPPDATA\OpenTron
         (override with $env:OpenTron_HOME).
      ...
      7. Optionally register the scheduled-task service (see
         deploy/windows/Tron-service.ps1).

    Usage (one-liner):
      irm https://open-Tron.github.io/OpenTron/install.ps1 | iex

    Usage (file invocation, supports flags):
      irm https://open-Tron.github.io/OpenTron/install.ps1 -OutFile install.ps1
    ...
    are honored via env vars when the corresponding flag is absent:
      $env:OpenTron_SKIP_SERVICE = '1'
      $env:OpenTron_SERVICE      = '1'
      $env:OpenTron_FORCE        = '1'
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:OpenTron_API_KEY (an
...
if (-not $SkipService -and $env:OpenTron_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:OpenTron_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:OpenTron_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but OpenTron requires
...
$installRoot = if ($env:OpenTron_HOME) {
    $env:OpenTron_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'OpenTron'
}
...
$repoUrl = if ($env:OpenTron_REPO_URL) {
    $env:OpenTron_REPO_URL
} else {
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'Tron' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. Tron.cmd shim - so bare `Tron` works in any new PowerShell
# ---------------------------------------------------------------------------

$binDir = Join-Path $installRoot 'bin'
$shimPath = Join-Path $binDir 'Tron.cmd'
...
# so the shim is self-locating - moving %LOCALAPPDATA%\OpenTron won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" Tron %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\OpenTron\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\OpenTron\bin` (unexpanded) into User
...
Write-Ok "Tron shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\Tron-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register OpenTron as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'OpenTron' registered (loopback default)."
}
...
Write-Host "  |   OpenTron install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'Tron' } else { 'Tron doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the Tron shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'Tron doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN OpenTron     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://open-Tron.github.io/OpenTron/"
Write-Host ""
