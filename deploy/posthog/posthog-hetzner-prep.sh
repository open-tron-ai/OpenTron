...
# OpenTron's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.OpenTron.ai you@OpenTron.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "OpenTron".
#   3. Settings → Project → grab the Project API Key (phc_…).
#   4. Update src/OpenTron/core/config.py AnalyticsConfig defaults:
#        host = "https://<DOMAIN>"
#        key  = "phc_<new>"
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.OpenTron.ai team@OpenTron.ai
...
Next steps:
  ...
  3. Create project "OpenTron".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/OpenTron/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of OpenTron with the new config defaults.
